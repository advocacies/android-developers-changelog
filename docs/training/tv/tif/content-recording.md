---
title: Support content recording  |  Android TV  |  Android Developers
...
# Support content recording Stay organized with collections Save and categorize content based on your preferences.
...
1. In your TV input service `onCreate()`
   method, create a new `TvInputInfo` object using the
   `TvInputInfo.Builder` class.
2. When creating the new `TvInputInfo` object, call
   `setCanRecord(true)` before calling
   `build()` to indicate that your
   service supports recording.
3. Register your `TvInputInfo` object with the system by calling
...
`notifyTuned()` or, if your app can't tune to the proper channel, call
`notifyError()`.
