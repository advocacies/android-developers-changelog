---
title: Sharing files  |  App data and files  |  Android Developers
...
# Sharing files Stay organized with collections Save and categorize content based on your preferences.
...
`FileProvider` component provides the method
`getUriForFile()` for
...
`Intent` that contains the data. To learn how to send simple data with an
`Intent`, see the training class
...
generated by the Android `FileProvider` component and
...
:   Learn how an app can use a content URI generated by a
    `FileProvider` to retrieve file information including
    MIME type and file size.

For additional related information, refer to:

* [Storage Options](/guide/topics/data/data-storage)
* [Saving Files](/training/basics/data-storage/files)* [Sharing Simple Data](/training/sharing)