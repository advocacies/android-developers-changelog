---
title: Navigation  |  Wear OS  |  Android Developers
...
* [Guides](https://developer.android.com/training/wearables/views)
...
![](/static/wear/images/screen_opts_2.png)
...
* You must implement the swipe-to-dismiss yourself. Otherwise, when the user performs a
  swipe, they exit the entire app.
* If you are using
  [`AmbientMode`](/reference/androidx/wear/ambient/AmbientMode), you must
  customize it to work properly. `AmbientMode` is set on the activity, so you have to
  take that into account when implementing fragments.
...
* Make sure every fragment uses a
  [`SwipeDismissFrameLayout`](/reference/kotlin/androidx/wear/widget/SwipeDismissFrameLayout)
  as its root, and manually use the dismiss action to go back in the navigation graph.
* Implement a custom
