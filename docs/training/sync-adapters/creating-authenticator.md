---
title: Create a stub authenticator  |  Connectivity  |  Android Developers
...
# Create a stub authenticator Stay organized with collections Save and categorize content based on your preferences.
...
`android:icon`
:   Pointer to a [Drawable](/guide/topics/resources/drawable-resource)
...
In a previous step, you created a bound `Service` that links the authenticator
...
the bound `Service` you have provided to wrap the authenticator.
...
provider; otherwise, go to the lesson [Creating a Sync Adapter](/training/sync-adapters/creating-sync-adapter).