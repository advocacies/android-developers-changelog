---
title: Developer guide  |  Android Enterprise  |  Android Developers
...
# Developer guide Stay organized with collections Save and categorize content based on your preferences.
...
* [Work profile compatibility](#work-profiles)—Modify your Android
  app so it functions best on a managed device.
* [Managed configurations](#managed-configurations)—Modify
  your app to allow IT admins the option to specify custom
  settings for your apps.
* [Dedicated devices](#dedicated-device)—Optimize your
  app so that it can be deployed on an Android device as a kiosk.
* [Single Sign-On (SSO)](#sso)—Simplify the sign-on process
...
[Work profiles](/work/managed-profiles)
...
* Device administration via the
  [DevicePolicyManager](/reference/android/app/admin/DevicePolicyManager) class

### Considerations for work profiles

* The Android system prevents intents
  [from crossing profiles](/reference/android/app/admin/DevicePolicyManager#clearCrossProfileIntentFilters(android.content.ComponentName)) and IT admins can
  [enable or disable system apps](/reference/android/app/admin/DevicePolicyManager#enableSystemApp(android.content.ComponentName,%20java.lang.String)).
...
[`Intent.resolveActivity()`](/reference/android/content/Intent#resolveActivity(android.content.pm.PackageManager)).
...
[Prevent Failed Intents](/work/managed-profiles#prevent_failed_intents).
...
|  |  |
| --- | --- |
| **Use:**  Content URIs | * The [content URIs](/reference/android/content/ContentUris) contain the authority, path, and ID for a   specific file. You can generate this using   [FileProvider](/reference/androidx/core/content/FileProvider) subclass.   [Learn more](/training/secure-file-sharing) * Share and grant permissions to access the content URI using   an Intent. Permissions can only be passed across the profile   boundary using Intents. If you grant another app access rights   to your file using   [`Context.grantUriPermission()`](/reference/android/content/Context#grantUriPermission(java.lang.String,%20android.net.Uri,%20int)), it only is granted for   that app in the same profile. |
...
profiles, test it in a work profile. See [Test your app](#testing).
...
The [Set up Managed Configurations](/work/managed-configurations)
...
[`getApplicationRestrictions()`](/reference/android/content/RestrictionsManager#getApplicationRestrictions()) returns:
...
* **A bundle with a single key value pair with
  [`KEY_RESTRICTIONS_PENDING`](/reference/android/os/UserManager#KEY_RESTRICTIONS_PENDING) set to true**—your
...
* **Listen while running**—Dynamically register
  [`ACTION_APPLICATION_RESTRICTIONS_CHANGED`](/reference/android/content/Intent#ACTION_APPLICATION_RESTRICTIONS_CHANGED) in your
...
[Custom Tabs](https://developer.android.com/topic/libraries/support-library/features.html#custom-tabs) are supported back to API level 15 (Android 4.0.3).
...
on GitHub](https://github.com/openid/AppAuth-Android).
...
* Install Test DPC from
  [GooglePlay](https://play.google.com/store/apps/details?id=com.afwsamples.testdpc).
...
[Test DPC User
Guide](https://github.com/googlesamples/android-testdpc).
...
4. Run the following [Android Debug Bridge](/studio/command-line/adb) (adb) command in
   your terminal:
...
5. Once you’ve completed provisioning the device owner, you can test your app on that device. You
   ...
   [managed configurations](/work/managed-configurations) and
   [intents](/work/managed-profiles#prevent_failed_intents) work on that device.

You can also use other provisioning methods—see the [Test DPC User Guide](https://github.com/googlesamples/android-testdpc). To learn how IT
admins typically enroll and provision Android-powered devices, read
[Provision
devices](https://developers.google.com/android/work/play/emm-api/prov-devices).
...
1. Create a test version of your application with a
   [new ApplicationId](http://tools.android.com/tech-docs/new-build-system/applicationid-vs-packagename).
2. Claim a [managed Google domain](https://support.google.com/work/android/answer/6174056) and bind it to your EMM. If you
...
3. [Publish your application to the private channel](https://support.google.com/a/answer/2494992) for their
