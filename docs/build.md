---
title: Configure your build  |  Android Studio  |  Android Developers
...
In [Gradle build overview](/studio/build/gradle-build-overview) and
[Android build structure](/studio/build/android-build-structure),
...
:   Product flavors represent different versions of your app that you can
    ...
    product flavors](/studio/build/build-variants#product-flavors).
...
:   A build variant is a cross-product of build type and product flavor and
    ...
    how to create and manage build variants, read the [Configure build variants](/studio/build/build-variants)
    overview.
...
:   You can specify values for some properties of the manifest file in the build
    ...
    merger tool [merges manifest settings](/studio/build/manage-manifests#merge-manifests).
...
:   The build system manages project dependencies from your local file system
    ...
    dependencies](/studio/build/dependencies).
...
:   The build system lets you specify signing settings in the build
    ...
    have a release key, you can generate one as described in [Sign your app](/studio/publish/app-signing). Signed release builds
...
:   The build system lets you specify a different ProGuard rules file for
    ...
    your code and resources](/studio/build/shrink-code) using its built-in shrinking tools, such as R8.
...
:   The build system lets you automatically build different APKs that
    ...
    For more information see [Build multiple APKs](/studio/build/configure-apk-splits). However, releasing a single AAB is
...
version for your build. See [Java versions in Android builds](/build/jdks)
for details.
...
[DSL reference documentation](/reference/tools/gradle-api). Kotlin script also relies on the
...
**Tip:** If simultaneously working on multiple projects, if possible, ensure that
all of the projects use the same Gradle version. Otherwise, Gradle creates
copies of the Gradle daemon for each Gradle version, in addition to separate
copies for each [JDK used to run Gradle](/build/jdks#jdk-gradle). This increases
memory and CPU usage, potentially slowing your builds or impacting other
work on the machine.
...
![Overview of SDK specifications in a Gradle build](/static/images/build/android-sdks.svg)
...
:   The `compileSdk` determines which Android and Java APIs are
    ...
    [conditionally guard use of newer features](/training/basics/supporting-devices/platforms#version-codes) or use
    [AndroidX compatibility libraries](/jetpack/androidx/releases/appcompat) to use newer features with lower
    Android API levels.

    Each Android SDK provides a subset of Java APIs for use in your application.
    The table at [Which Java APIs can I use in my Java or Kotlin source code](/build/jdks#compileSdk)
    ...
    [desugaring](/studio/write/java8-support), which must be
    ...
    [`minorApiLevel`](/reference/tools/gradle-api/current/com/android/build/api/dsl/CompileSdkVersion#minorApiLevel):
...
:   The `minSdk` specifies the lowest version of Android that you
    ...
    [making newer features conditional](/training/basics/supporting-devices/platforms#version-codes) or by using
    [`Appcompat`](/jetpack/androidx/releases/appcompat) for backward compatibility.
...
:   The `targetSdk` serves two purposes:
    ...
    distribution policy enforces [additional policies on target API level](/google/play/requirements/target-sdk).
...
:   Configures local environment properties for the build system, including the
    ...
    * `ndk.dir` - Path to the NDK. This property has been
      ...
      `ndk` directory within the Android SDK directory.
    * `sdk.dir` - Path to the Android SDK.
    * `cmake.dir` - Path to CMake.
    * `ndk.symlinkdir` - In Android Studio 3.5 and higher,
      creates a symlink to the NDK that can be shorter than the installed
      NDK path.

**Caution:** The `local.properties` file is reserved for
properties specific to the Android Gradle plugin. Putting your own values in
this file can cause problems. If you need to define your own local properties,
[create a separate properties file and manually load it](/studio/build/gradle-tips#remove-private-signing-information-from-your-project).
...
figure 2, or click **Sync Project** ![](/static/studio/images/buttons/toolbar-sync-gradle.png)
...
![](/static/images/tools/as-gradlesync-updated.png)


**Figure 2.** Sync the project with build configuration
files in Android Studio.

**Note:** When Android Studio detects build configuration changes that haven't been
synced yet it disables run configurations to avoid confusing failures. If you
want to run or debug anyway without syncing, select **Ignore these changes** to
re-enable those actions.
...
:   Create this source set to include code and resources only for a specific
    ...
    **Note:** If you configure your build to [combine multiple
    product flavors](/studio/build/build-variants#flavor-dimensions), you can create source set directories for each
...
manifests](/studio/build/manage-manifests#merge-manifests), Gradle uses the same priority order so each build variant can
define different components or permissions in the final manifest. To learn
more about creating custom source sets, read [Create source sets](/studio/build/build-variants#sourcesets).
...
[known issues](https://github.com/bazelbuild/bazel/issues?q=is%3Aissue+is%3Aopen+label%3Ateam-Android).