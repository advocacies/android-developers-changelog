---
title: Multiple APK support  |  Other Play guides  |  Android Developers
...
# Multiple APK support Stay organized with collections Save and categorize content based on your preferences.
...
Multiple APK support is a feature on Google Play that allows you to publish
...
devices with a single APK.

Android-powered devices may differ in several ways and it's important to the
...
To help you publish your application for as many devices as possible, Google
...
configuration support you've declared in the manifest file of each APK.
...
* Support different CPU architectures with each APK (such as for ARM or
  x86, when your app uses the [Android
  NDK](/tools/sdk/ndk)).
* Optimize for entry-level devices
  such as those running Android (Go edition).
...
**Note:** To learn about preparing and publishing
APKs on Google Play, see the
[Prepare & roll-out releases](https://support.google.com/googleplay/android-developer/answer/7159011) support article.
...
Which devices receive each APK is determined by [Google Play filters](/google/play/filters) that are specified by
...
* **OpenGL texture compression formats**
  ...
  This is based on your manifest file's [`<supports-gl-texture>`](/guide/topics/manifest/supports-gl-texture-element) element(s).
...
* **Screen size (and, optionally, screen density)**
  ...
  This is based on your manifest file's [`<supports-screens>`](/guide/topics/manifest/supports-screens-element)
  *or* [`<compatible-screens>`](/guide/topics/manifest/compatible-screens-element)
  element. You should never use both elements and you should use only [`<supports-screens>`](/guide/topics/manifest/supports-screens-element)
  when possible.
  ...
  [Build Multiple APKs](/studio/build/configure-apk-splits).

  Consider the following best practices to support all screen sizes:
  + The Android system provides strong support for applications to support
    ...
    [Supporting Multiple
    Screens](/guide/practices/screens_support) so that your application is flexible and can adapt to all
    screen configurations with a single APK.
  + By default, all screen size attributes in the
    [`<supports-screens>`](/guide/topics/manifest/supports-screens-element) element are "true" if you do
    not declare them otherwise. However, because the
    `android:xlargeScreens` attribute was added in Android 2.3
    (API level 9), Google Play will assume that it is "false" if your
    application does not set either
    [`android:minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min) or
    [`android:targetSdkVersion`](/guide/topics/manifest/uses-sdk-element#target) to "9" or higher.
  + You should not combine both
    [`<supports-screens>`](/guide/topics/manifest/supports-screens-element) and
    [`<compatible-screens>`](/guide/topics/manifest/compatible-screens-element) elements in your manifest
    ...
    [Distributing to
    Specific Screens](/guide/practices/screens-distribution). If you can't avoid using both, be aware that for
    any conflicts in agreement between a given size, "false" will win.
* **Device feature sets**

  This is based on your manifest file's [`<uses-feature>`](/guide/topics/manifest/uses-feature-element)
  element(s).
  ...
  [Features
  Reference](/guide/topics/manifest/uses-feature-element#features-reference) for a list of features supported by the platform.
* **Android (Go edition)**

  To target devices running Android (Go edition),
...
* **API level**
  ...
  This is based on your manifest file's [`<uses-sdk>`](/guide/topics/manifest/uses-sdk-element)
  element. You can use both the [`android:minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min)
  and [`android:maxSdkVersion`](/guide/topics/manifest/uses-sdk-element#max)
  ...
  APIs, go to [Configure Product Flavors](/studio/build/build-variants#product-flavors).

  If you use this characteristic as the factor to distinguish multiple
  APKs, then the APK with a higher
  [`android:minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min) value must have a higher
  [`android:versionCode`](/guide/topics/manifest/manifest-element#vcode) value. This is also true if two
  ...
  further in the section below about [Rules for multiple
  APKs](#Rules).

  You should avoid using
  [`android:maxSdkVersion`](/guide/topics/manifest/uses-sdk-element#max) in general, because as long as
  ...
  [`android:minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min) is `"16"` in one APK
  and `"21"` in another, devices that support API level 21 or
...
* **CPU architecture (ABI)**
  ...
  architectures, or [Application Binary
  Interfaces (ABIs)](/ndk/guides/abis). Instead of packaging all available libraries into one
  ...
  target ABI, go to [Build
  Multiple APKs](/studio/build/configure-apk-splits).

Other manifest elements that enable [Google Play filters](/google/play/filters)—but are not
...
example, you cannot provide different APKs that differ purely on the [`<uses-configuration>`](/guide/topics/manifest/uses-configuration-element) characteristics.

### Rules for multiple APKs

Before you publish multiple APKs for your application, you need to
...
* Each APK **must have a different version code**, specified by the
  [`android:versionCode`](/guide/topics/manifest/manifest-element#vcode) attribute.
* Each APK **must not exactly match the configuration support of another APK**.

  That is, each APK must declare slightly different support for at least one of
  the [supported Google Play filters](#SupportedFilters) (listed above).
  ...
  that overlap range will receive the APK with a higher version code (defined by [`android:versionCode`](/guide/topics/manifest/manifest-element#vcode)).
* You cannot activate a new APK that has a version code lower than that of
  the APK it's replacing. For example, say you have an active APK for screen
  sizes small - normal with version code `0400`, then try to replace
  it with an APK for the same screen sizes with version code `0300`.
...
* An APK that requires a **higher API level** must have a **higher
  ...
  supported API levels (no other [supported filters](#SupportedFilters)
  ...
  **Note:** The size of the version code increase is irrelevant; it
  ...
  + If an APK you've uploaded for API levels 16 and above (Android 4.1.x+) has a version code of
    `0400`, then an APK for API levels 21 and above (Android 5.0+) must be `0401` or
  ...
  + If you have one APK that's for API level 16 (and above) *and* small -
    large screens, and another APK for API level 21 (and above) *and* large - xlarge screens, then
    ...
    APK.
  + If you have one APK that's for API level 16 (and above) *and* small -
    normal screens, and another APK for API level 21 (and above) *and* large - xlarge
  ...
  + If you have one APK that's for API level 16 (and above) *and* ARMv7 CPUs,
    and another APK for API level 21 (and above) *and* ARMv5TE CPUs,
...
Failure to abide by the above rules results in an error on the Google Play Console
...
application code is to use a [library project](/tools/projects#LibraryProjects). A library
...
example, "HelloWorld\_21" might be a good name for an application designed for
...
sure you also understand each of the [Rules for multiple APKs](#Rules).

### Assigning version codes

Each APK for the same application **must have a unique version code**, specified by
the [`android:versionCode`](/guide/topics/manifest/manifest-element#vcode) attribute. You must be careful about assigning version codes when
...
more information about how this requirement applies, see the section above about [Rules for multiple APKs](#Rules).
...
for a target device, you may want to also create a *universal* APK
...
`versionCode`. Because Google Play Store installs the version of
...
user visible version assigned to [`android:versionName`](/guide/topics/manifest/manifest-element#vname)),
...
the supported configurations are in the higher order bits, and the version name (from [`android:versionName`](/guide/topics/manifest/manifest-element#vname)) is in the lower order bits. For example, when the application version
...
![](/static/images/market/version-codes.png)
...
arrow\_forward](/training/multiple-apks/api)