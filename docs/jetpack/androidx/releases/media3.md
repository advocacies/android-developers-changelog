---
title: Media3  |  Jetpack  |  Android Developers
...
API Reference  
[androidx.media3.common](/reference/kotlin/androidx/media3/common/package-summary)

Support libraries for media use cases.

| Latest Update | Stable Release | Release Candidate | Beta Release | Alpha Release |
| --- | --- | --- | --- | --- |
| September 10, 2026 | [1.11.1](/jetpack/androidx/releases/media3#1.11.1) | - | - | - |

## Declaring dependencies

To add a dependency on Media3, you must add the Google Maven repository to your
...
Your feedback helps make Jetpack better. You can use the
[Media3 issue tracker](https://github.com/androidx/media/issues) to find answers
...
* ExoPlayer:
  ...
  + Fix race condition related to audio session id usage where the initial
    ...
    ([#3241](https://github.com/androidx/media/issues/3241)).
...
Use the 1.11.0 [stable version](#1.11.0).
...
Use the 1.10.0 [stable version](#1.10.0).
...
* Effect:
  + Explicitly set `GL_DRAW_BUFFER` and `GL_READ_BUFFER` when
    transitioning from surfaceless context.

### 1.9.3

March 16, 2026

Version 1.9.3 contains the following
[commits](https://github.com/androidx/media/commits/1.9.3).
...
Version 1.9.2 contains the following
[commits](https://github.com/androidx/media/commits/1.9.2).
...
Version 1.9.1 contains [these
commits](https://github.com/androidx/media/commits/1.9.1).
...
* Extractors:
  ...
  + MP3: Change `Mp3Extractor` to default to a constant bitrate (CBR)
    ...
    specification history, where CBR was standard and VBR requires
...
* IMA extension:
  ...
  + Add `ImaServerSideAdInsertionUriBuilder.setNetworkCode`, a new API for
    ...
    the network code, see [this
    article](http://support.google.com/admanager/answer/7674889).
  + Bump IMA dependency to 3.37.0 which requires enabling core library
    ...
    [config
    notes](https://developers.google.com/interactive-media-ads/docs/sdks/android/client-side/get-started#2.-add-the-ima-sdk-to-your-project).
    \* Support IMA DAI custom UI options in SSAI URI builder. Custom UI
    options for server side ad insertion include "Skippable" and
    "About This Ad" rendering support. The feature is currently
    available for selected publishers behind an allow list. This
    change also upgrades the IMA SDK version to 3.38.0 ([release
    notes](https://developers.google.com/interactive-media-ads/docs/sdks/android/client-side/history))
...
* Session:
  ...
  + Fix bug where `KEYCODE_HEADSETHOOK` did not start the player upon and
    ...
    by handling 'KEYCODE\_HEADSETHOOK' just like `KEYCODE_MEDIA_PLAY_PAUSE`
...
* Cast extension:
  ...
  + Use `MediaItem.mediaMetadata.mediaType` to infer the Cast MEDIA\_TYPE to
...
Use the 1.9.0 [stable version](#1.9.0).
...
Version 1.8.0 contains [these
commits](https://github.com/androidx/media/commits/1.8.0).
...
* Video:
  ...
  + Add experimental `ExoPlayer` API to include the
    ...
    \* Improve codec performance checks for software video codecs. This
    may lead to some additional tracks being marked as
...
* Cast extension:
  ...
  + Prevent CastPlayer from entering STATE\_BUFFERING while the timeline is
    empty.
...
Use the 1.8.0 [stable version](#1.8.0).
...
`androidx.media3:media3-*:1.6.1` is released. Version 1.6.1 contains [these
commits](https://github.com/androidx/media/commits/1.6.1).
...
`androidx.media3:media3-*:1.6.0` is released. Version 1.6.0 contains [these
commits](https://github.com/androidx/media/commits/1.6.0).
...
* Transformer:
  ...
  + Enable support for Android platform diagnostics using
    ...
    be collected by Google [if sharing usage and diagnostics data is
    enabled](https://support.google.com/accounts/answer/6078260) by the user
    of the device. Apps can opt-out of contributing to platform diagnostics
...
* Video:
  ...
  + Add experimental `ExoPlayer` API to drop late `MediaCodecVideoRenderer`
    ...
    \* Fix issue where a player without a surface was ready immediately
    and very slow decoding any pending frames
...
* Session:
  + Keep foreground service state for an additional 10 minutes when playback
    ...
    devices. Note that just calling `player.pause()` can no longer be used
  ...
  + Improve handling of Android platform MediaSession actions ACTION\_PLAY
    and ACTION\_PAUSE to only set one of them according to the available
...
* Decoder extensions (FFmpeg, VP9, AV1, etc.):
  + Add the MPEG-H decoder module which uses the built-in MPEG-H decoder to
    decode MPEG-H audio
...
* Remove deprecated symbols:
  ...
  + Removed the following deprecated `DownloadHelper` methods:
    ...
    - Removed
      ...
      and pass in `(MediaSession.Token) compatToken.getToken()` instead.

### 1.6.0-rc02

March 19, 2025

Use the 1.6.0 [stable version](#1.6.0).
...
`androidx.media3:media3-*:1.5.1` is released. Version 1.5.1 contains [these
commits](https://github.com/androidx/media/commits/1.5.1).
...
`androidx.media3:media3-*:1.5.0` is released. Version 1.5.0 contains [these
commits](https://github.com/androidx/media/commits/1.5.0).
...
* Common Library:
  ...
  + Remove `@DoNotInline` annotations from manually out-of-lined inner
    classes designed to avoid [runtime class verification
    ...
    Gradle Plugin that uses a version of R8 which does this, [due to
    `compileSdk = 35`](https://issuetracker.google.com/345472586#comment7).
    ...
    failures. This change has [already been done in other AndroidX
...
* Text:
  + Add a custom `VoiceSpan` and populate it for [WebVTT voice spans](https://www.w3.org/TR/webvtt1/#webvtt-cue-voice-span)
...
* Session:
  ...
  + Add command buttons for media items. This adds the Media3 API for what
    ...
    [Implement custom browse
    actions](/training/cars/media/create-media-browser/custom-browse-actions).
    \* Fix bug where a Media3 controller was sometimes unable to let a
    session app start a foreground service after requesting
    `play()`.
...
Use the 1.5.0 [stable version](#1.5.0).
...
`androidx.media3:media3-*:1.4.1` is released. Version 1.4.1 contains [these
commits](https://github.com/androidx/media/commits/1.4.1).
...
`androidx.media3:media3-*:1.4.0` is released. Version 1.4.0 contains [these
commits](https://github.com/androidx/media/commits/1.4.0).
...
* Text:
  ...
  + Change default subtitle parsing behavior so it happens during extraction
    instead of during rendering (see [ExoPlayer's architecture
...
* Effect:
  ...
  + Add support for EGL\_GL\_COLORSPACE\_BT2020\_HLG\_EXT, which improves HLG
...
* IMA extension:
  + Promote API that is required for apps to play [DAI ad streams](https://developers.google.com/ad-manager/dynamic-ad-insertion/full-service)
    to stable.
...
Use the 1.4.0 [stable version](#1.4.0).
...
`androidx.media3:media3-*:1.3.1` is released. Version 1.3.1 contains [these
commits](https://github.com/androidx/media/commits/1.3.1).
...
* Decoder Extensions (FFmpeg, VP9, AV1, MIDI, etc.):
  + Disable the MIDI extension as a local dependency by default because it
    ...
    this module from a local dependency [can re-enable
    it](https://github.com/androidx/media/blob/main/README.md#midi-module).

### Version 1.3.0

March 6, 2024

`androidx.media3:media3-*:1.3.0` is released. Version 1.3.0 contains [these
commits](https://github.com/androidx/media/commits/1.3.0).
...
* Common Library:
  ...
  + Define ads with a full `MediaItem` instead of a single `Uri` in
    `AdPlaybackState`.
  + Increase `minSdk` to 19 (Android KitKat). This is [aligned with all
    other AndroidX
...
* ExoPlayer:
  ...
  + Prevent subclassing `CompositeSequenceableLoader`. This component was
    ...
    by wrapping an instance using the [decorator
    pattern](https://en.wikipedia.org/wiki/Decorator_pattern) and
...
Use the [1.3.0 stable version](#1.3.0).
...
* Extractors:
  ...
  + Add `WebpExtractor`.
  + Add `HeifExtractor`.
  + Add [QuickTime classic](https://developer.apple.com/standards/qtff-2001.pdf)
...
Use the 1.2.0 [stable version](#1.2.0).
...
Use the 1.1.0 [stable version](#1.1.0).
...
`androidx.media3:media3-*:1.0.2` is released. [Version 1.0.2 contains these
commits.](https://github.com/androidx/media/commits/1.0.2)

This release corresponds to the [ExoPlayer 2.18.7
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.7).

This release contains the following changes since the [1.0.1 release](#1.0.1):
...
`androidx.media3:media3-*:1.0.1` is released. [Version 1.0.1 contains these
commits.](https://github.com/androidx/media/commits/1.0.1)

This release corresponds to the [ExoPlayer 2.18.6
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.6).
...
`androidx.media3:media3-*:1.0.0` is released. [Version 1.0.0 contains these
commits.](https://github.com/androidx/media/commits/1.0.0)

This release corresponds to the [ExoPlayer 2.18.5
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.5).
...
`androidx.media3:media3-*:1.0.0-rc02` is released. [Version 1.0.0-rc02 contains
these commits.](https://github.com/androidx/media/commits/1.0.0-rc02)

This release corresponds to the [ExoPlayer 2.18.4
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.4).
...
`androidx.media3:media3-*:1.0.0-rc01` is released. [Version 1.0.0-rc01 contains
these commits.](https://github.com/androidx/media/commits/1.0.0-rc01)

This release corresponds to the [ExoPlayer 2.18.3
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.3).
...
`androidx.media3:media3-*:1.0.0-beta03` is released. [Version 1.0.0-beta03
contains these commits.](https://github.com/androidx/media/commits/1.0.0-beta03)

This release corresponds to the [ExoPlayer 2.18.2
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.2).
...
`androidx.media3:media3-*:1.0.0-beta02` is released. [Version 1.0.0-beta02
contains these commits.](https://github.com/androidx/media/commits/1.0.0-beta02)

This release corresponds to the [ExoPlayer 2.18.1
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.1).
...
`androidx.media3:media3-*:1.0.0-beta01` is released. [Version 1.0.0-beta01
contains these commits.](https://github.com/androidx/media/commits/1.0.0-beta01)

This corresponds to the [ExoPlayer 2.18.0
release](https://github.com/google/ExoPlayer/releases/tag/r2.18.0).
...
* Core library:
  + Enable support for Android platform diagnostics via
    ...
    enabled](https://support.google.com/accounts/answer/6078260) by the user
    of the device. Apps can opt-out of contributing to platform diagnostics
    for ExoPlayer with `ExoPlayer.Builder.setUsePlatformDiagnostics(false)`.
...
* Track selection:
  ...
  + Add
    ...
    will monitor for changes in the [Spatializer
    properties](https://developer.android.com/reference/android/media/Spatializer.OnSpatializerStateChangedListener)
    and trigger a new track selection upon these. Devices with a
    `television` [UI
    mode](https://developer.android.com/guide/topics/resources/providing-resources#UiModeQualifier)
...
`androidx.media3:media3-*:1.0.0-alpha03` is released. [Version 1.0.0-alpha03
contains these
commits.](https://github.com/androidx/media/commits/1.0.0-alpha03)

This corresponds to the [ExoPlayer 2.17.1
release](https://github.com/google/ExoPlayer/releases/tag/r2.17.1).
...
`androidx.media3:media3-*:1.0.0-alpha02` is released. [Version 1.0.0-alpha02
contains these
commits.](https://github.com/androidx/media/commits/1.0.0-alpha02)

This corresponds to the [ExoPlayer 2.17.0
release](https://github.com/google/ExoPlayer/releases/tag/r2.17.0).
...
* DRM:
  + Remove `playbackLooper` from `DrmSessionManager.(pre)acquireSession`.
    ...
    instead.
* Ad playback / IMA:
  + Add support for [IMA Dynamic Ad Insertion (DAI)](https://support.google.com/admanager/answer/6147120)
...
`androidx.media3:media3-*:1.0.0-alpha01` is released. [Version 1.0.0-alpha01
contains these
commits.](https://github.com/androidx/media/commits/1.0.0-alpha01)

#### New features

Media3 is the new home for media support libraries, including ExoPlayer. The
first alpha contains early, functional implementations of libraries for
...
For more information, see the [Media3 GitHub
project](https://github.com/androidx/media).

ExoPlayer was previously hosted in a separate [ExoPlayer GitHub
project](https://github.com/google/ExoPlayer). In Media3 its package name is
`androidx.media3.exoplayer`. We plan to continue to maintain and release the
ExoPlayer GitHub project for a while to give apps time to migrate to Media3.
Media3 has replacements for all the ExoPlayer modules, except for the legacy
media2 and mediasession extensions, which are replaced by the new
