---
title: Emoji2  |  Jetpack  |  Android Developers
...
API Reference  
[androidx.emoji2.text](/reference/kotlin/androidx/emoji2/text/package-summary)  
[androidx.emoji2.viewshelper](/reference/kotlin/androidx/emoji2/viewshelper/package-summary)  
[androidx.emoji2.widget](/reference/kotlin/androidx/emoji2/widget/package-summary)

Display emoji in current and older devices.

| Latest Update | Stable Release | Release Candidate | Beta Release | Alpha Release |
| --- | --- | --- | --- | --- |
| September 09, 2026 | [1.6.0](/jetpack/androidx/releases/emoji2#1.6.0) | [1.7.0-rc01](/jetpack/androidx/releases/emoji2#1.7.0-rc01) | - | - |
...
[existing issues](https://issuetracker.google.com/issues?q=componentid:460938%20status:open)
...
`androidx.emoji2:emoji2-*:1.7.0-rc01` is released. Version 1.7.0-rc01 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/8a1952bf7de9742b8d81da766a2849d2c51f8803..dc5da9e3ab63099a55bab9baf115d72000763773/emoji2).

This release focuses on performance improvements and is recommended for all users of emoji2.
...
`androidx.emoji2:emoji2-*:1.7.0-alpha01` is released. Version 1.7.0-alpha01 contains [these commits](https://android.googlesource.com/platform/frameworks/support/+log/9ed4b1acacae02bd637e1932def67751b89244cb..f0365b8b5cca21e7455f5aa90a214d326b76c1e7/emoji2).

\*\* Performance Release \*\*

This release is a major performance improvement over prior relases.

• **Reduced memory footprint:** Metadata trie RAM usage is reduced from ~2.2 MB to ~80 KB.  
• **Reduced startup overhead:** Startup object allocations are reduced from 24,178 to 82.  
• **Allocation-free text scanning:** Scanning standard Latin text now performs zero allocations.  
• **Reduced latency:** Median Latin text scan latency is reduced by up to 83% (from 16.43 µs to 2.66 µs).  
• **Automatic bypass on API 35+:** EmojiCompat processing is automatically bypassed on devices with system-updatable fonts.

**API Changes**

* Added `setUseAfterUpdatableSystemFonts` to `EmojiCompat.Config` to optionally continue using `EmojiCompat` on Android 15 (API 35) and above even when updatable system fonts are available on all devices. The default value will disable EmojiCompat on API35+ saving memory and text processing latency. ([I7235e](https://android-review.googlesource.com/#/q/I7235ede8e0d03de06c4f9b0860cf028947bd433e), [b/541363642](https://issuetracker.google.com/issues/541363642))

**Bug Fixes**

* Emoji2 picker - Updated Compose compileSdk to API 37. This means that a minimum AGP version of 9.2.0 is required when using Compose. ([Id45cd](https://android-review.googlesource.com/#/q/Id45cdca34ef948e06259b2dd9adc901b7c930492), [b/413674743](https://issuetracker.google.com/issues/413674743))
...
2. Inflate the emoji picker view and optionally set `emojiGridRows` and `emojiGridColumns` based on the desired size of each emoji cell
   ...
   * You could set `emojiGridRows` as a float to indicate there’s more emojis if scroll down
     in XML
...
* This release allows [Compose Foundation `1.4.0`](/jetpack/androidx/releases/compose-foundation#1.4.0) and higher to enable emoji2 integration.
...
* EmojiCompat correctly sets `EditorInfo.extras` on Android 11
  ...
  + Custom widgets that use IME not subclassing EditText may call
    `EmojiCompat.updateEditorInfo` to inform IME that they support
    EmojiCompat processing. ([I1ea9b](https://android-review.googlesource.com/#/q/I1ea9bdd579e64139d98bc1298ad2d9c5ade65045))

**Bug Fixes**

* Fix `DefaultEmojiCompatConfig` to correctly lookup emoji font
  provider on API 19 and 28. This fixes a bug introduced in emoji2
  1.0.0-alpha01. ([Ib33d8](https://android-review.googlesource.com/#/q/Ib33d84de2a030f6e57e3d5382a088255a202692a), [b/197906329](https://issuetracker.google.com/issues/197906329))
...
* Allow null KeyListener in AppCompatEditText. This reverses the
  non-null annotation that was added to AppCompatEditText in 1.4.0-alpha01
  and restores the previous behavior when passed null. ([I21482](https://android-review.googlesource.com/#/q/I214824131c0206349b73471a8c22be38bf5dd0d8), [b/189559345](https://issuetracker.google.com/issues/189559345))

**Bug Fixes**

* Change EmojiCompatInitializer to delay font loading until
  500ms after the first `Activity.onResume`. This allows an activity to
  perform `Application.onCreate` and `Activity.onCreate` uncontended, while
  still ensuring that the emoji font is loaded shortly after app startup. ([I4bff7](https://android-review.googlesource.com/#/q/I4bff7a1b8ae23529be059f5154a8de91ba939861))
...
* Renamed package in `emoji2-views-helper` to
  `androidx.emoji2.viewsintegration`. This is a breaking change for
  AppCompat `1.4.0-alpha01`, and apps must ensure AppCompat dependency is updated to use
  the new emoji2 version. ([Ie8397](https://android-review.googlesource.com/#/q/Ie83972affa2281616923620e1642964eead49b4e))
...
`emoji2` is added as a dependency to AppCompat starting in [AppCompat `1.4.0-alpha01`](/jetpack/androidx/releases/appcompat#1.4.0-alpha01) and is enabled by default for AppCompat views.
...
* Apps with AppCompat should upgrade to appcompat version [AppCompat `1.4.0-alpha01`](/jetpack/androidx/releases/appcompat#1.4.0-alpha01) or higher.
