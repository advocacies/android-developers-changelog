---
title: ViewPager2  |  Jetpack  |  Android Developers
...
[User Guide](/guide/navigation/navigation-swipe-view-2)  [Code Sample](https://github.com/android/views-widgets-samples/tree/main/ViewPager2)

API Reference  
[androidx.viewpager.adapter](/reference/kotlin/androidx/viewpager2/adapter/package-summary)  
[androidx.viewpager.widget](/reference/kotlin/androidx/viewpager2/widget/package-summary)
...
| --- | --- | --- | --- | --- |
| May 14, 2024 | [1.1.0](/jetpack/androidx/releases/viewpager2#1.1.0) | - | - | - |

## AndroidX Dependencies

To use `ViewPager2`, add the following AndroidX dependency to your project's
`build.gradle` file:
...
[existing issues](https://issuetracker.google.com/issues?q=componentid:561920%20status:open)
...
* Fixes crashes when used with [RecyclerView `1.3.1-rc01`](/jetpack/androidx/releases/recyclerview#recyclerview-1.3.1-rc01) or higher.
* `ViewPager2` now correctly populates the `CollectionInfo` and `CollectionItemInfo` that [RecyclerView `1.2.0-alpha02`](/jetpack/androidx/releases/recyclerview#recyclerview-1.2.0-alpha02) and higher no longer populate by default.
...
* Move to targeting Java 8 for all androidx libraries
  ([2923f39](https://android-review.googlesource.com/#/q/2923f39afb2f019e043f3a8e1d409fa49d90808e))

**Bug Fixes**

* Fixed dispatch of window insets, all pages now get the same
  insets. ([I47fef](https://android-review.googlesource.com/#/q/I47fef39b5fca4a936c8428b741097974b56a518c))
...
This release accompanies a change in [RecyclerView 1.2.0-alpha02](/jetpack/androidx/releases/recyclerview#recyclerview-1.2.0-alpha02) by populating `CollectionInfo` and `CollectionItemInfo` that `RecyclerView` no longer provides by default. When updating to `RecyclerView 1.2.0-alpha02` also update `ViewPager2` to prevent Accessibility regressions.
...
* Improvements from the previous [ViewPager](/reference/androidx/viewpager/widget/ViewPager) implementation:
...
* Easy migration from the previous `ViewPager` implementation (API parity where possible). See the [migration guide](/training/animation/vp2-migration) and the [sample app](http://goo.gle/viewpager2-sample).

See the [guide](/training/animation/screen-slide-2) on using ViewPager2 to slide between Fragments.
...
* An issue in [SwipeRefreshLayout](/jetpack/androidx/releases/swiperefreshlayout#1.1.0-alpha03) was fixed to work better with ViewPager2.
...
* A number of issues were fixed in other components to work better with `ViewPager2`: [RecyclerView](/jetpack/androidx/releases/recyclerview#1.1.0-beta04), [NestedScrollView](/jetpack/androidx/releases/core#1.2.0-alpha04), and [Navigation](/jetpack/androidx/releases/navigation#2.2.0-alpha02).
...
* We are still working on the [remaining open issues](https://issuetracker.google.com/issues?q=hotlistid:814487%20status:open%20targetedto:(6.1%20%7C%206.3)) before moving to Stable
