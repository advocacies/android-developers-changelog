---
title: Enable and customize cross-device handoff  |  Better Together  |  Android Developers
...
# Enable and customize cross-device handoff Stay organized with collections Save and categorize content based on your preferences.
...
basis. To enable the feature, call [`setHandoffEnabled()`](/reference/kotlin/android/app/Activity#setHandoffEnabled(kotlin.Boolean,%20android.app.HandoffActivityParams)) to denote the
...
[`isHandoffEnabled()`](/reference/kotlin/android/app/Activity#isHandoffEnabled()). If an activity has enabled handoff, it must also
implement [`onHandoffActivityDataRequested()`](/reference/kotlin/android/app/Activity#onHandoffActivityDataRequested(android.app.HandoffActivityDataRequestInfo)) to ensure it returns a
non-null [`HandoffActivityData`](/reference/kotlin/android/app/HandoffActivityData), as no default behavior is implemented.
...
* The activity is in the foreground, and handoff has been requested. You can
  check the status of a handoff request.

## Check status

The [`HandoffActivityDataRequestInfo`](/reference/kotlin/android/app/HandoffActivityDataRequestInfo) class provides information about the
current request to hand off an activity. Use [`isActiveRequest()`](/reference/kotlin/android/app/HandoffActivityDataRequestInfo#isActiveRequest()) to
determine whether the request is currently active. If the method returns

* true, the current activity is in the foreground and the user has requested
  to Continue On another device, however if
* false, the activity has stopped and is in the background, but the user may
  request a handoff in the future.
...
* **Component name** (required): The [`ComponentName`](/reference/kotlin/android/content/ComponentName) of a corresponding
  activity to launch on the receiving device to resume the user's journey
  after handoff. In most cases and for exact activity replication, this will
  be the component name for the same activity creating the
  `HandoffActivityData`.
* **Extras** (optional): You can specify extras that will be included in the
  start [`Intent`](/reference/kotlin/android/content/Intent) of the activity. This is a [`PersistableBundle`](/reference/kotlin/android/os/PersistableBundle) that
  can be used to provide additional information to the component. Extras must
  be less than 50KB.

**Important:** Because `HandoffActivityData` is passed between devices, DO NOT
include sensitive or personally identifiable information (for example, user
credentials).

To create `HandoffActivityData` objects, use the builder class,
[`HandoffActivityData.Builder`](/reference/kotlin/android/app/HandoffActivityData.Builder). You can construct a new builder with either
the [`Activity`](/reference/kotlin/android/app/Activity) or `ComponentName` of the activity that should receive the
...
device, you can additionally use [`setFallbackUri()`](/reference/kotlin/android/app/HandoffActivityData.Builder#setFallbackUri(android.net.Uri)) to optionally set a
...
[`HandoffActivityData.createWebHandoff()`](/reference/kotlin/android/app/HandoffActivityData#createWebHandoff(android.net.Uri)) method to generate the
`HandoffActivityData`.
