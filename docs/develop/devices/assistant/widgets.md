---
title: Integrate App Actions with Android widgets  |  Assistant  |  Android Developers
...
# Integrate App Actions with Android widgets Stay organized with collections Save and categorize content based on your preferences.
...
**Figure 1.** Launching a widget for
...
[Android app widget](/guide/topics/appwidgets/overview) in Google Assistant to fulfill these kinds of intents.
...
[Widgets Extension library](#library).
...
1. **Discovery:** Proactively display widgets in response to users'
   natural language queries.
2. **Engagement:** Display widgets in hands-free contexts, such as when
   Assistant provides [personal results](//support.google.com/assistant/answer/7684543) on the lock screen, and on
   [Android Auto](/cars).
3. **Retention:** Let users pin the widgets displayed in
   Assistant to their launcher. Pinning functionality requires the
   [Widgets Extension library](#library).
...
* Speaking a query to Assistant that triggers a
  [built-in intent](/reference/app-actions/built-in-intents) (BII) or [custom intent](/guide/app-actions/custom-intents) configured for
  widget fulfillment.
...
widget whenever a user triggers the [`GET_EXERCISE_OBSERVATION`](/reference/app-actions/built-in-intents/health-and-fitness/get-exercise-observation) BII in your
...
* **Parameter access:** Assistant provides the [intent parameters](/guide/app-actions/intents#implement_biis_and_handle_intent_parameters) extracted
  from the user query to your widget, enabling
  [tailored responses](#extract_parameters).
* **Custom TTS introductions:** You can provide a text-to-speech (TTS) string
  for Assistant to announce when displaying your widget.
* **Widget pinning:** Assistant displays an **Add this widget** button near
  your widget, letting users easily pin your widgets to their launcher.

**Note:** Not all BIIs support widget fulfillment. Refer to the
[BII reference](/reference/app-actions/built-in-intents) for the BII you plan to use.

## Implement widget fulfillment

To implement widget fulfillment for your intents, follow these steps:

1. Implement an Android widget by following the steps described in
   [Create a simple widget](/guide/topics/appwidgets).
2. In your app's `shortcuts.xml` resource file, add an
   [`<app-widget>`](#app-widget) element to your capability containing
   fulfillment details and BII [`<parameter>`](#parameter) tags. Update your
   widget to handle the parameters.
3. Add the required [Widgets Extension library](#library), which lets
   Assistant pass BII names and parameters to your widgets. It also enables
   [custom TTS introductions](#tts) and widget [pinning](#pinning)
   functionality.
...
[`<app-widget>`](#app-widget) elements are defined as fulfillments within
[`<capability>`](/guide/app-actions/action-schema#capability) elements in `shortcuts.xml`. They require the following
attributes, unless noted as optional:

| `shortcuts.xml` tag | Contained in | Attributes |
| --- | --- | --- |
| [`<app-widget>`](#app-widget) | [`<capability>`](/guide/app-actions/action-schema#capability) | * `android:identifier` * `android:targetClass` |
| [`<parameter>`](#parameter) | [`<app-widget>`](#app-widget) | * See [App Actions schema](/guide/app-actions/action-schema#parameter) for attributes. |
| [`<extra>`](#extra) | [`<app-widget>`](#app-widget) | * `android:name` (only applicable for TTS) * `android:value` (optional) |
...
* `android:identifier`: the identifier for this fulfillment. This value must
  be unique across the `<app-widget>` and `<intent>` fulfillment
  elements defined within a `<capability>`.
* `android:targetClass`: the full class name of the [`AppWidgetProvider`](/reference/android/appwidget/AppWidgetProvider) to
  handle the intent.
...
You access these extras by calling [`getAppWidgetOptions()`](/reference/android/appwidget/AppWidgetManager#getAppWidgetOptions(int)) on the associated
`AppWidgetManager` object, which returns a [`Bundle`](/reference/android/os/Bundle) containing the name of
the triggering BII and its parameters. See
[Extract parameter values](#extract_parameters) for details.

For more information on BII parameter matching, see
[Parameter data and matching](/guide/app-actions/action-schema#url-templates-in-fulfillment).

#### <extra>

Optional tag declaring that a [custom TTS introduction](#tts) is used for
...
[`GET_EXERCISE_OBSERVATION`](/reference/app-actions/built-in-intents/health-and-fitness/get-exercise-observation) BII capability:
...
[Fallback intents](#guideline_fallback) section for more information
about defining fallback intents.

**Note:** Assistant fulfills App Actions for capabilities containing multiple
fulfillment options by evaluating each fulfillment element in the order it
was defined. Assistant fulfills the action by selecting the first fulfillment
whose required `<parameter>` values are fully provided in the user query.
...
visually rendered to users. It also enables [launcher pinning](#pinning), making
...
Next, use the library to define your introduction strings and update the
widget, as shown in this `ExampleAppWidget`:
...
BIIs supporting [inline inventory](/guide/app-actions/inline-inventory) or [web inventory](/guide/app-actions/web-inventory) can extend these
inventories to your widget fulfillments.

#### Inline inventory

The following code from a sample `shortcuts.xml` file demonstrates a
[`START_EXERCISE`](/reference/app-actions/built-in-intents/health-and-fitness/start-exercise) BII capability
...
Android Studio](/guide/app-actions/test-tool), to test widgets on a physical or virtual device. To use
...
5. Use the Assistant app on your test device to test your App Action. For
   example, you can say something like *"Hey Google, how many miles have I run
   this week on ExampleApp?"*
6. Observe the behavior of your app, or use the [Android Studio debugger](/studio/debug), to
   verify the desired action result.
...
* Focus widget content completely on fulfilling the intent. Don't
  try to fulfill multiple intents with one widget or add irrelevant content.

#### Handle authentication

* (**Required**) Where user authentication is needed to complete a user flow,
  return a widget that explains that the user needs to continue in the app.
  Inline user authentication in Google Assistant is not supported for App
  Actions.
* If users permit your app to show data using widgets, you can return an
  error widget at runtime for unauthorized users.
...
[App actions](/guide/app-actions/overview) might also
