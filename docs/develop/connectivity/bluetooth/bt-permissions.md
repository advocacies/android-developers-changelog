---
title: Bluetooth permissions  |  Connectivity  |  Android Developers
...
# Bluetooth permissions Stay organized with collections Save and categorize content based on your preferences.
...
technologies, you can [check for availability at runtime](#availability).
...
[Companion
Device Manager (CDM)](/develop/connectivity/bluetooth/companion-device-pairing) provides a more streamlined method of connecting to
...
![Bluetooth permissions dialog](/static/images/develop/connectivity/bluetooth/nearby-devices.svg)


System permissions dialog, asking the user to
grant an app permission to discover, advertise, and connect to nearby
devices.
...
1. If your app [looks for Bluetooth
   devices](/develop/connectivity/bluetooth/find-bluetooth-devices), such as
   BLE peripherals, declare the
...
2. If your app [makes the current device discoverable to other Bluetooth
   devices](/develop/connectivity/bluetooth/find-bluetooth-devices#enable-discoverability),
   declare the
...
3. If your app [communicates with already-paired Bluetooth
   devices](/develop/connectivity/bluetooth/transfer-data), declare the
   [`BLUETOOTH_CONNECT`](/reference/android/Manifest.permission#BLUETOOTH_CONNECT)
   permission.
4. For your legacy Bluetooth-related permission declarations, set
   `android:maxSdkVersion` to 30. This app compatibility step helps the system
   grant your app only the Bluetooth permissions that it needs when installed on
   devices that run Android 12 or higher.
5. If your app uses Bluetooth scan results to derive physical location, declare
   the
   [`ACCESS_FINE_LOCATION`](/reference/android/Manifest.permission#ACCESS_FINE_LOCATION)
   permission. Otherwise, you can [strongly assert that your app doesn't derive
   physical location](#assert-never-for-location) and set `android:maxSdkVersion`
   to 30 for the `ACCESS_FINE_LOCATION` permission.

The `BLUETOOTH_ADVERTISE`, `BLUETOOTH_CONNECT`, and `BLUETOOTH_SCAN` permissions
are [runtime permissions](/develop/permissions/overview#runtime).
Therefore, you must explicitly [request user
approval](/training/permissions/requesting) in your app before you can look for
...
1. Add the `android:usesPermissionFlags` attribute to your `BLUETOOTH_SCAN`
   ...
   **Note:** If you include `neverForLocation` in your
   `android:usesPermissionFlags`, some BLE beacons are filtered from the scan
   results.
...
* [`BLUETOOTH`](/reference/android/Manifest.permission#BLUETOOTH) is necessary
  to perform any Bluetooth classic or BLE communication, such as requesting a
  connection, accepting a connection, and transferring data.
* [`ACCESS_FINE_LOCATION`](/reference/android/Manifest.permission#ACCESS_FINE_LOCATION)
  is necessary because, on Android 11 and lower, a Bluetooth scan
  could potentially be used to gather information about the location of the
  user.
...
requiring the location permission. For more on this option, see [Companion
device pairing](/develop/connectivity/bluetooth/companion-device-pairing).

Because location permissions are [runtime permissions](/guide/topics/permissions/overview#runtime),
you must [request these permissions at runtime](/training/permissions/requesting)
...
[`BLUETOOTH_ADMIN`](/reference/android/Manifest.permission#BLUETOOTH_ADMIN)
...
See the [`<uses-permission>`](/guide/topics/manifest/uses-permission-element)
...
[`<uses-feature>`](/guide/topics/manifest/uses-feature-element) element allows
...
[`PackageManager.hasSystemFeature()`](/reference/android/content/pm/PackageManager#hasSystemFeature(java.lang.String)):
