---
title: Reducing image download sizes  |  Views  |  Android Developers
...
# Reducing image download sizes Stay organized with collections Save and categorize content based on your preferences.
...
Android apps typically use images that are in one or more of the following file
formats: AVIF, PNG, JPG, and WebP. For each of these formats, there are steps you can
take to reduce image sizes.

### AVIF

Android 12 (API level 31) and higher support images that use the AV1 Image File Format (AVIF). AVIF is a
container format for images and sequences of images encoded using AV1. AVIF takes advantage of the
intra-frame encoded content from video compression. This dramatically improves image quality for the
same file size when compared to older image formats, such as JPEG. For an in-depth look at the
advantages of this format, see Jake Archibald's
[blog post](https://jakearchibald.com/2020/avif-has-landed/).
...
![](/static/topic/performance/images/beforeafterindexed.png)
...
![](/static/topic/performance/images/palette.png)
...
single *center point* value, which minimizes error for colors in that
...
![](/static/topic/performance/images/vq.gif)
...
![](/static/topic/performance/images/parrot.png)
...
[26% smaller](https://developers.google.com/speed/webp/docs/webp_lossless_alpha_study#conclusions) than PNGs. These image files also support
transparency (also known as alpha channel) at a cost of just
[22% more](https://developers.google.com/speed/webp/docs/webp_lossless_alpha_study#results) bytes.

WebP lossy images are
[25-34% smaller](https://developers.google.com/speed/webp/docs/webp_study#experiment_1_webp_vs_jpeg_at_equal_ssim_index) than comparable JPG images at equivalent
...
[WebP site](https://developers.google.com/speed/webp/).

You can convert existing BMP, JPG, PNG or static GIF images to WebP format using
Android Studio. For more information, see [Create WebP
images Using Android Studio](/studio/write/convert-webp).
...
![](/static/topic/performance/images/comparison.png)
...
[majority of devices](https://developer.android.com/about/dashboards/index.html#Platform) satisfy that requirement.

Figure 6 provides a simple visualization to help you decide which compression
scheme to use.

![](/static/topic/performance/images/decisions.png)
...
![](/static/topic/performance/images/moarparrots.png)
...
[App Engine](https://cloud.google.com/appengine/) comes
