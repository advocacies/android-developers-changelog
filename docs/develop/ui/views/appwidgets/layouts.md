---
title: Provide flexible widget layouts  |  Views  |  Android Developers
...
# Provide flexible widget layouts Stay organized with collections Save and categorize content based on your preferences.
...
[determine a size for your widget](#anatomy_determining_size).
...
1. [Specify additional widget sizing constraints.](#specify-widget-size-constraints)
2. Providing [responsive layouts](#provide-responsive-layouts) or [exact
   layouts.](#provide-exact-layouts)

In previous versions of Android, it is possible to get the size ranges of a
widget using the
[`OPTION_APPWIDGET_MIN_WIDTH`](/reference/android/appwidget/AppWidgetManager#OPTION_APPWIDGET_MIN_WIDTH),
[`OPTION_APPWIDGET_MIN_HEIGHT`](/reference/android/appwidget/AppWidgetManager#OPTION_APPWIDGET_MIN_HEIGHT),
[`OPTION_APPWIDGET_MAX_WIDTH`](/reference/android/appwidget/AppWidgetManager#OPTION_APPWIDGET_MAX_WIDTH),
and [`OPTION_APPWIDGET_MAX_HEIGHT`](/reference/android/appwidget/AppWidgetManager#OPTION_APPWIDGET_MAX_HEIGHT)
...
providing [responsive](#provide-responsive-layouts) or [exact
layouts](#provide-exact-layouts).
...
In addition to the existing [`minWidth`](/reference/android/appwidget/AppWidgetProviderInfo#minWidth),
[`minHeight`](/reference/android/appwidget/AppWidgetProviderInfo#minHeight),
[`minResizeWidth`](/reference/android/appwidget/AppWidgetProviderInfo#minResizeWidth),
and [`minResizeHeight`](/reference/android/appwidget/AppWidgetProviderInfo#minResizeHeight)
attributes, use the following new `appwidget-provider` attributes:

* [`targetCellWidth`](/reference/android/appwidget/AppWidgetProviderInfo#targetCellWidth)
  and [`targetCellHeight`](/reference/android/appwidget/AppWidgetProviderInfo#targetCellHeight):
...
* [`maxResizeWidth`](/reference/android/appwidget/AppWidgetProviderInfo#maxResizeWidth)
  and [`maxResizeHeight`](/reference/android/appwidget/AppWidgetProviderInfo#maxResizeHeight):
...
size at runtime](#provide-exact-layouts), as described in this page.
...
* `wideView` supports from 215dp × 110dp (`minResizeHeight`) to 250dp
  ...
  **Note:** It's possible for the widget's size to be larger than
  `maxResizeWidth` × `maxResizeHeight`. See [Widget sizing
  attributes](/guide/topics/appwidgets#widget-sizing-attributes) for more details.
...
![Example of responsive layout](/static/images/appwidgets/size-range.gif)
...
1. Overload [`AppWidgetProvider.onAppWidgetOptionsChanged()`](/reference/android/appwidget/AppWidgetProvider#onAppWidgetOptionsChanged(android.content.Context,%20android.appwidget.AppWidgetManager,%20int,%20android.os.Bundle)),
   which is called when the set of sizes changes.
2. Call [`AppWidgetManager.getAppWidgetOptions()`](/reference/android/appwidget/AppWidgetManager#getAppWidgetOptions(int)),
   which returns a [`Bundle`](/reference/android/os/Bundle) containing the sizes.
3. Access the [`AppWidgetManager.OPTION_APPWIDGET_SIZES`](/reference/android/appwidget/AppWidgetManager#OPTION_APPWIDGET_SIZES) key from the `Bundle`.

**Note:** Providing the list of sizes is the launcher’s responsibility. If the
device has a launcher that doesn't support that field, the list of sizes
...
**Note:** The preceding table shows the available sizes on a Google Pixel 4 to
demonstrate the rough estimation of general available sizes in both portrait and
landscape modes.
...
**Note:** As mentioned previously, it's possible for the widget's size to be larger
than `maxResizeWidth` x `maxResizeHeight`. See [Widget sizing
attributes](/guide/topics/appwidgets#widget-sizing-attributes) for more details.
...
* The mapping between the number of cells and the available size in dps
  follows the [table showing estimation of minimum
  dimensions](#estimate-minimum-dimensions) in this page.
...
![Example weather widget in a 4x2 'medium' size. Resizing the widget
            ...
            4pm through 7pm.](/static/images/appwidgets/weather-size-4x2.png)
...
![Example weather widget in a 5x2 'medium' size. Resizing the widget
            ...
            stretched by one cell length to take up more horizontal space.](/static/images/appwidgets/weather-size-5x2.png)
...
![Example weather widget in a 5x3 'large' size. Resizing the widget
            ...
            and high and low temperatures for each day.](/static/images/appwidgets/weather-size-5x3.png)
...
![Example weather widget in a 5x4 'large' size. Resizing the widget
            ...
            for each day).](/static/images/appwidgets/weather-size-5x4.png)
