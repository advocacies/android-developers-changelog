---
title: Stack traces in Compose  |  Jetpack Compose  |  Android Developers
...
* Composition
* `DisposableEffect` and `LaunchedEffect` (except for `onDispose` or
  cancellation)
...
**Warning:** Verify that you are using Kotlin 2.3.0+ and have R8 enabled when using
`Auto` or `GroupKeys` option in production builds.
...
There are four options for the [`ComposeStackTraceMode`](/reference/kotlin/androidx/compose/runtime/tooling/ComposeStackTraceMode):

* [`Auto`](/reference/kotlin/androidx/compose/runtime/tooling/ComposeStackTraceMode#Auto()): Recommended option, as it uses `GroupKeys`
  if the app is minified and `None` otherwise.
* [`GroupKeys`](/reference/kotlin/androidx/compose/runtime/tooling/ComposeStackTraceMode#GroupKeys()): Stack traces are created for
  minified apps. The group key information is retained even after minification
  and is used together with the proguard mapping file emitted by Compose
  compiler and R8 to reconstruct a rough location of `@Composable` functions.
  These stack traces are less precise, and optimized to avoid doing additional
  work at runtime. Compose compiler supports emission of additional R8
  mappings starting from Kotlin
  2.3.0.
* [`SourceInformation`](/reference/kotlin/androidx/compose/runtime/tooling/ComposeStackTraceMode#SourceInformation()): Useful for non-minified
  builds, collects source information and adds it to the stack trace. The
  results are more accurate, but incur significant performance cost that is
  similar to attaching the Layout inspector. They are created to be used in
  debug versions of the apps to get accurate readings on a crash that requires
  more information about its location. The source information is removed from
  minified apps to optimize the binary size and performance.
* [`None`](/reference/kotlin/androidx/compose/runtime/tooling/ComposeStackTraceMode#None()): No extra stack trace details added.
...
Trace app performance](/develop/ui/compose/tooling/tracing)