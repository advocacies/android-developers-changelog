---
title: Copy and paste  |  Jetpack Compose  |  Android Developers
...
this document assumes some familiarity with the [Android Content Provider API](/guide/topics/providers/content-providers).
...
Refer to [Handle keyboard actions](/develop/ui/compose/touch-input/keyboard-input/commands) for details.

### Copy with `ClipboardManager`

You can copy texts to the clipboard with [`ClipboardManager`](/reference/kotlin/androidx/compose/ui/platform/ClipboardManager).
Its [setText()](/reference/kotlin/androidx/compose/ui/platform/ClipboardManager#setText(androidx.compose.ui.text.AnnotatedString)) method copies
...
A common use case is [copying sensitive content](#sensitive_content),
such as password. [`ClipEntry`](/reference/kotlin/androidx/compose/ui/platform/ClipEntry) describes an item on the clipboard.
It contains a [`ClipData`](/reference/android/content/ClipData) object that describes data on the clipboard.
[`ClipData.newPlainText()`](/reference/android/content/ClipData#newPlainText(java.lang.CharSequence,%20java.lang.CharSequence)) method is a convenience method to
create a `ClipData` object from a String object.
You can set the created `ClipEntry` object to clipboard
by calling the [setClip()](/reference/kotlin/androidx/compose/ui/platform/ClipboardManager#setClip(androidx.compose.ui.platform.ClipEntry)) method
...
by calling [`getText()`](/reference/kotlin/androidx/compose/ui/platform/ClipboardManager#getText()) method
...
The [`contentReceiver`](/reference/kotlin/androidx/compose/ui/Modifier#(androidx.compose.ui.Modifier).contentReceiver(androidx.compose.foundation.content.ReceiveContentListener)) modifier helps you to implement pasting rich content.

### Copy rich content

Your app can't copy rich content directly to the clipboard.
Instead, your app passes a `URI` object to the clipboard
and provides access to the content with a [`ContentProvider`](/reference/android/content/ContentProvider).
The following code snippet shows how to copy a JPEG image to clipboard.
Refer to [Copy data streams](/develop/ui/views/touch-and-input/copy-paste#Streams) for details.
...
With the [`contentReceiver`](/reference/kotlin/androidx/compose/ui/Modifier#(androidx.compose.ui.Modifier).contentReceiver(androidx.compose.foundation.content.ReceiveContentListener)) modifier, you can handle pasting rich content
to [`BasicTextField`](/reference/kotlin/androidx/compose/foundation/text/BasicTextField.composable) in the modified component.
The following code snippet adds the pasted URI of an image data
to a list of [`Uri`](/reference/android/net/Uri) objects.
...
The `contentReceiver` modifier takes a [`ReceiveContentListener`](/reference/kotlin/androidx/compose/foundation/content/ReceiveContentListener) object
as its argument and calls [`onReceive`](/reference/kotlin/androidx/compose/foundation/content/ReceiveContentListener#onReceive(androidx.compose.foundation.content.TransferableContent))
method of the passed object when the user pastes data
to the `BasicTextField` inside the modified component.

A [`TransferableContent`](/reference/kotlin/androidx/compose/foundation/content/TransferableContent) object is passed to the onReceive method,
...
A `ClipEntry` object can have several [`ClipData.Item`](/reference/android/content/ClipData.Item) objects
...
The [`TransferableContent.hasMediaType()`](/reference/kotlin/androidx/compose/foundation/content/TransferableContent#(androidx.compose.foundation.content.TransferableContent).hasMediaType(androidx.compose.foundation.content.MediaType)) method can help you determine
...
Refer to [Use content providers to copy complex data](/develop/ui/views/touch-and-input/copy-paste#Provider) for details.
...
The actual data can be retrieved from a [`ContentProvider`](/reference/android/content/ContentProvider).
Refer to [Retrieve data from the provider](/guide/topics/providers/content-provider-basics#SimpleQuery) for more information.
...
You can learn more about this edge case in [Avoid duplicate notifications](/develop/ui/views/touch-and-input/copy-paste#duplicate-notifications)).
...
See the [recommendation](/develop/ui/views/touch-and-input/copy-paste#Feedback).
...
You must add a flag to [`ClipDescription`](/reference/android/content/ClipDescription) in `ClipData`
