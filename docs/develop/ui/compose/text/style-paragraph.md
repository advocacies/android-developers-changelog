---
title: Style paragraph  |  Jetpack Compose  |  Android Developers
...
[alignment](/reference/kotlin/androidx/compose/ui/text/style/TextAlign) of the
...
![The words ](/static/develop/ui/compose/images/text-centered.png)
...
positioning a Composable within a container such as a `Row` or `Column`. For more
information, see [Compose layout basics](/develop/ui/compose/layouts/basics).

## Add multiple styles in a paragraph

To add multiple styles in a paragraph, you can use [`ParagraphStyle`](/reference/kotlin/androidx/compose/ui/text/ParagraphStyle) in an
[`AnnotatedString`](/reference/kotlin/androidx/compose/ui/text/AnnotatedString), which can be annotated with styles of arbitrary annotations.
...
[Add multiple styles in text](/develop/ui/compose/text/style-text#multiple-styles).

`AnnotatedString` has a [type-safe
builder](https://kotlinlang.org/docs/reference/type-safe-builders.html)
to make it easier to create: [`buildAnnotatedString`](/reference/kotlin/androidx/compose/ui/text/package-summary#buildAnnotatedString(kotlin.Function1)). The following snippet uses
...
![Three paragraphs in three different styles: Blue, red and bold, and plain black](/static/develop/ui/compose/images/text-paragraph-styles.png)

## Adjust line height and padding

[`includeFontPadding`](/reference/android/widget/TextView#attr_android:includeFontPadding) is a legacy property that adds extra padding based on
...
The ability to configure [`lineHeight`](/reference/android/widget/TextView#attr_android:lineHeight) is not new– it has been available
...
text. You can then use the new [`LineHeightStyle API`](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle) to further configure
how this text is aligned within the space, and remove whitespace.

You may want to adjust [`lineHeight`](/reference/kotlin/androidx/compose/ui/text/TextStyle#lineHeight()) using the text unit "em" (relative font
size) instead of "sp" (scaled pixels) for improved precision. For more information about
selecting an appropriate text unit, see [`TextUnit`](/reference/kotlin/androidx/compose/ui/unit/TextUnit).

![Image showing lineHeight as a measurement based on the lines directly above and below it.](/static/develop/ui/compose/images/text-lineheightv3.png)
...
using configurations with the [`LineHeightStyle` API](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle): [`LineHeightStyle.Alignment`](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Alignment)
and [`LineHeightStyle.Trim`](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Trim) (`includeFontPadding` must be set to `false` for
...
[`LineHeightStyle.Alignment`](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Alignment) defines how to align the line in the space
...
various [`LineHeightStyle.Trim`](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle.Trim) configurations when alignment is centered
...
**Warning:** These adjustments using the `LineHeightStyle` API (Trim) can only work
when you use them along with the configuration `includeFontPadding = false.`
...
system, and the changes made for Compose and the new [`LineHeightStyle`](/reference/kotlin/androidx/compose/ui/text/style/LineHeightStyle)
APIs.

## Insert line breaks

The [`LineBreak`](/reference/kotlin/androidx/compose/ui/text/style/LineBreak) API defines the criteria by which text is split across
...
* `Heading` — Line breaking with looser breaking rules. Recommended for short
  text, such as titles.
* `Paragraph` — Slower, higher-quality line breaking for improved readability.
  Recommended for larger amounts of text, such as paragraphs.
...
line breaking strategy has more variability in line lengths.](/static/develop/ui/compose/images/text/simple-paragraph.png)


**Figure 1.** A text block with a simple line breaking strategy (top) versus a
text block with paragraph-optimized line breaking (bottom).
...
* `Balanced` — Attempts to balance the line lengths of the text, also applying
  automatic hyphenation if enabled. Recommended for small screens, like watches,
  to maximize the amount of text displayed.
* `HighQuality` — Optimizes a paragraph for more readable text, including
  ...
  `Balanced` or `Simple`.)
* `Simple` — basic, fast strategy. If enabled, hyphenation is done only for
  words that don't fit on an entire line by themselves. Useful for editing text
  to avoid changing positions while typing.

**Note:** You can also customize your line breaking behavior with the `Strictness`
and `WordBreak` parameters. These parameters are designed for CJK languages, and
you may not see the intended effect if using them for non-CJK languages. For
more information, see [CJK considerations](#cjk-considerations).
...
![A paragraph with a balanced line breaking strategy and a paragraph
  ...
  strategy has more consistent line lengths than the default.](/static/develop/ui/compose/images/text/balanced-default.png)


**Figure 2.** A paragraph formatted with a `Balanced` line-breaking strategy
(top) versus a paragraph formatted without a line-breaking strategy.

### CJK considerations

You can also customize `LineBreak` with the [`Strictness`](/reference/kotlin/androidx/compose/ui/text/style/LineBreak.Strictness) and
[`WordBreak`](/reference/kotlin/androidx/compose/ui/text/style/LineBreak.WordBreak) APIs, which were **designed specifically for CJK languages**.
...
* `Default` — Default breaking rules for the locale. May correspond to `Normal`
  or `Strict`.
...
**Note:** To test the following snippet, choose the Japanese locale on your device
...
![Japanese text with Strictness and WordBreak settings versus default text.](/static/develop/ui/compose/images/text/cjk-line-break.png)


**Figure 3.** Text formatted with `Strictness` and `WordBreak` settings (top)
versus text formatted only with `LineBreak.Heading` (bottom).

## Hyphenate text split across lines

The [`Hyphens`](/reference/kotlin/androidx/compose/ui/text/style/Hyphens) API lets you add hyphenation support to your app.
...
![A paragraph without hyphenation enabled and a paragraph with hyphenation enabled.
  When hyphenation is enabled, a word is hyphenated and split across two lines.](/static/develop/ui/compose/images/text/hyphens.png)


**Figure 4.** A paragraph without hyphenation enabled (top) versus a paragraph
with hyphenation enabled (bottom).

When enabled, hyphenation only occurs under the following conditions:

* A word does not fit onto a line. If you use a `Simple` line breaking strategy,
  the hyphenation of a word occurs only if a line is shorter than the single
  word.
* The appropriate locale is set on your device, as appropriate hyphenation is
  determined using dictionaries present on the system.
