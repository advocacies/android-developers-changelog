---
title: Optimize your app battery using Android vitals wake lock metric  |  Android Developers' Blog
...
![warning.png](/static/blog/assets/warning_acc52c9e53_29d4gO.webp)
...
Wake locks created by [audio](/develop/background-work/background-tasks/awake/wakelock/identify-wls#media), [location](/develop/background-work/background-tasks/awake/wakelock/identify-wls#location), and [JobScheduler](/develop/background-work/background-tasks/awake/wakelock/identify-wls#jobs) user initiated APIs are exempted from the wake lock calculation.
...
* The app manually acquires and releases the wake lock using [PowerManager](/reference/android/os/PowerManager) APIs for a specific use case, often this is acquired in conjunction with a [Foreground Service](/develop/background-work/services/fgs) - a platform lifecycle API intended for user-perceptible operation.
...
![wakelock.png](/static/blog/assets/wakelock_503e30ff8a_Zt4oMN.webp)

*Flowchart to decide when to manually acquire a wake lock*

1. Does the screen need to stay on?
   * Yes: Reference the [Keep Screen On](/develop/background-work/background-tasks/awake/screen-on) documentation instead
...
4. Is there already an API keeping the device awake on your behalf?
   * You can leverage the documentation [Identify wake locks created by other APIs](/develop/background-work/background-tasks/awake/wakelock/identify-wls) to identify scenarios where wake locks created by other APIs to identify scenarios where wake locks are created by other APIs such as LocationManager.
...
* **WorkManager:** Increase the periodic interval in [PeriodicWorkRequest](/reference/androidx/work/PeriodicWorkRequest)s.
* **SensorManager:** Leverage batching by specifying [maxReportLatencyMs](/reference/android/hardware/SensorManager#registerListener(android.hardware.SensorEventListener,%20android.hardware.Sensor,%20int,%20int)) when registering the listener.
* **Fused Location Provider:**
  ...
  + Use [setPriority(PRIORITY\_PASSIVE](https://developers.google.com/android/reference/com/google/android/gms/location/LocationRequest.Builder#public-locationrequest.builder-setpriority-int-priority)) for a less battery-intensive update method.
...
You can view more details in the [wake lock best practices documentation](/develop/background-work/background-tasks/awake/wakelock/best-practices).
...
The Android vitals excessive partial wake lock dashboard provides breakdowns of non-exempted wake lock names associated with your app, showing affected sessions and durations. Reminder to use the [documentation](/develop/background-work/background-tasks/awake/wakelock/identify-wls) to help you identify if the wake lock name is app-held or held by another API.

![breakdowns2.png](/static/blog/assets/breakdowns2_987ee7d6f2_1HmfEt.webp)
...
The full list of variations of worker-held wake lock names is available in [documentation](/develop/background-work/background-tasks/awake/wakelock/identify-wls#wake_lock_names_6). To debug these wake locks, you can use Background Task Inspector to debug locally, or leverage getStopReason to debug issues in the field.

**Android Studio Background Task Inspector**

![taskinspector.png](/static/blog/assets/taskinspector_60a38ee510_Z1iDBkV.webp)
...
See [Background Task Inspector documentation](/studio/inspect/task) for more details.

**WorkManager getStopReason**

For in-field debugging of workers with excessive wake locks, use [`WorkInfo.getStopReason()`](/reference/androidx/work/WorkInfo#getStopReason()) on WorkManager 2.9.0+ or for JobScheduler, [`JobParameters.getStopReason()`](/reference/android/app/job/JobParameters#getStopReason()) available on SDK 31+.
...
See [Optimize battery use for task scheduling APIs for more details](/develop/background-work/background-tasks/optimize-battery#why-stopped).
...
* Utilizing the [system trace command line tool](/topic/performance/tracing/command-line)
* Using the [Android Studio CPU Profiler](/studio/profile)
* Using the [Perfetto UI](https://perfetto.dev/docs/getting-started/system-tracing)
* Recording a trace manually on the device [directly from the developer options](/topic/performance/tracing/on-device).

![powermgmt.png](/static/blog/assets/powermgmt_72d69d505f_1A6Uxu.webp)
...
![perfetto.png](/static/blog/assets/perfetto_d7ad363bc9_2vnHBF.webp)
...
Check out the [ProfilingManager documentation](/topic/performance/tracing/profiling-manager/overview) for further steps on how to implement in field system trace collection which include how to programmatically [capture a trace](/topic/performance/tracing/profiling-manager/how-to-capture), [analyze profiling data](/topic/performance/tracing/profiling-manager/retrieve-and-analyze), and use [local debug commands](/topic/performance/tracing/profiling-manager/debug-mode).
