---
title: Create an Android library  |  Android Studio  |  Android Developers
...
* AAR files can contain Android resources and a manifest file, which lets
  you bundle in shared resources like layouts and drawables in addition
  to Kotlin or Java classes and methods.
* AAR files can contain [C/C++
  libraries](/studio/build/dependencies#using-native-dependencies)
  for use by the app module's C/C++ code.

A library module is useful in the following situations:

* When building multiple apps that use some of
  the same components, such as activities, services, or UI layouts
* When building an app that exists in multiple APK variations, such as
  a free and paid version, that share core components
...
[Publish your library](/build/publish-library)
...
2. In the **Create New Module** dialog that appears,
...
3. Give your library a name and select a minimum SDK version for the code
   ...
   **Important:** Module package names must be globally unique. You can't have two modules with
   the same package name in the same project.

Once the Gradle project sync completes, the library module appears in
the **Project** pane. If you don't see the new module
folder, make sure the pane is displaying the [**Android** view](/studio/projects#ProjectFiles).
...
1. Open the module-level `build.gradle` file, if you're using
   Groovy, or the `build.gradle.kts` file, if you're using Kotlin
   script.
...
3. In the **Declared Dependencies** tab, click
   ![](/static/studio/images/buttons/ic_plus.png) and select
   **Module Dependency** from the menu.
     
     
   ![](/static/studio/images/projects/psd-add-module-dependency-dropdown.png)
...
repositories](/studio/build/dependencies#remote-repositories).
...
**Note:** If you can't publish your library to a Maven
repository, you can consume the JAR or AAR file directly using the process
described in the [Add your AAR or JAR as a dependency](#psd-add-aar-jar-dependency)
section. In this case, you must manually manage any transitive dependencies of
that AAR.
...
1. Navigate to
   **File >
   Project Structure >
   Dependencies**.
2. In the **Declared Dependencies** tab, click
   ![](/static/studio/images/buttons/ic_plus.png) and select
   **Library Dependency** in the menu.
     
     
   ![](/static/studio/images/projects/psd-add-library-dependency-dropdown.png)
...
1. Navigate to **File >
   ...
   Dependencies**.
2. In the **Declared Dependencies** tab, click
   ![](/static/studio/images/buttons/ic_plus.png) and select **Jar
   Dependency** in the menu.
     
     
   ![](/static/studio/images/projects/psd-add-jar-dependency-dropdown.png)
3. In the **Add Jar/Aar Dependency** dialog, enter the path to your AAR
   ...
   **implementation** configuration.
   ...
   Dialog](/static/studio/images/projects/psd-add-aar-dependency.png)
...
**Note:** In the previous examples, the `implementation` configuration adds the
library as a build dependency for the entire app module. To learn how to
configure dependencies only for specific build variants, see
[Configure build variants](/studio/build/build-variants).
...
and the [lint tool](/studio/write/lint) warns you when you try
...
* **Libraries are merged in priority order.**
  ...
  * **A library module can depend on an external JAR library.**
    ...
    declare the external library in their manifest files in a [`<uses-library>`](/guide/topics/manifest/uses-library-element) element.
  ...
  * **A library module might include its own ProGuard configuration
    ...
    app](/studio/build/shrink-code).
  * **Testing a library module is nearly the same as
    [testing an app.](/studio/test)**
    ...
    When [merging
    multiple manifest files](/studio/build/manage-manifests#merge-manifests), Gradle follows the default priority order and
...
* `/jni/abi_name/name.so`
  (where abi\_name is one of the
  [Android-supported ABIs](/ndk/guides/abis#sa))
* `/proguard.txt`
* `/lint.jar`
* `/api.jar`
* `/prefab/` for [exporting native
  libraries](/studio/build/dependencies#native-dependencies-aars)