...
[dynamic color](https://m3.material.io/styles/color/dynamic-color/overview)
in your app, you can now use the new **System UI Mode** selector on the toolbar
...
Minimize the number of interruptions when developing by streaming your device
...
disclosure for devices connected with the `adb tcpip` command because the
...
Starting with Android Studio Flamingo Canary 1, the Network Inspector shows
all traffic data for the full timeline by default. You can select a range within
the timeline to see only the traffic in that range.

You can also create and manage rules that help test how your app behaves when
encountering different responses such as status codes, and response headers and
bodies. The rules determine what responses to intercept and how to modify these
responses before they reach the app. You can choose which rule to enable or
...
rule and include information about the origin of the response you want to
intercept under the **Origin** subsection. The URL in the **Rules** table should
...
You can also create sub-rules to modify the body of the response. You can choose
to either Find and Replace a section of the body, in which the first instance in
the body is replaced; or, you can choose to replace the entire content of the
body by selecting **Replace entire body**.

Similar to **Header rules**, you can create multiple body rules that are applied
in the order that they are listed in the table.
...
Android Studio Flamingo Canary 5 and higher introduce multiple new App Quality
...
To help you identify the most important issues, you can now filter by the
...
In addition, filters with lots of options are now searchable so you can
customize your view faster, without scrolling through all the menu options.
...
an API can exist in multiple SDKs. For example, an API introduced in Android U
can also be added to the Android R, S, and T backport extensions. Furthermore,
apps can specify multiple APIs as required using their manifest, and there are
new APIs for checking whether individual extensions are available. The lint tool
now handles the logic to check for overlap in what's required and what's known
to be present.

Finally, there are Studio IDE features associated with API checking. This
feature can now handle API violations for the new API surfaces, perform
necessary checks, and register quick fixes, one per available API surface.

## IntelliJ IDEA 2022.2 platform update

Android Studio Flamingo Canary 1 includes the IntelliJ IDEA 2022.2 updates,
which improve the IDE experience. For details on the changes, see the
[IntelliJ IDEA 2022.2 release notes](https://www.jetbrains.com/idea/whatsnew/2022-2/).

## Android Studio bundled with JDK 17

Starting from Android Studio Flamingo Canary 3, the Studio IDE is bundled with
JDK 17. If Android Studio is configured to use the embedded JDK, new projects
will use the latest stable version of the Android Gradle plugin and JDK 17.
