...
[system health](#3-5-system-health),
[feature polish](#3-5-feature-polish), and fixing bugs.
...
[community contributors](#3-5-community-contributors) who have
...
see [Maximum heap size](/studio/intro/studio-config#adjusting_heap_size).

![A notification about recommended memory settings.](/static/studio/images/memory-settings-notification.png)
...
report](/studio/report-bugs#run-memory-usage-report).
...
[Minimize the impact of antivirus software on build speed](/studio/intro/studio-config#antivirus-impact).
...
To learn more, see [Apply Changes](/studio/run#apply-changes).

![The toolbar buttons for Apply Changes.](/static/studio/images/releases/apply-changes-buttons-3.5.png)
...
![Target device drop-down menu.](/static/studio/images/run/deploy-run-app.png)
...
The **Build** ![Build window icon](/static/studio/images/buttons/window-build-2x.png)
...
![Constraint relationships for a selected UI element.](/static/studio/images/releases/constraint-relationships-3.5.png)
...
![An animation showing how to use the constraint widget to create
            constraints.](/static/studio/images/releases/constraint-widget-3.5.gif)
...
![An animation showing how to create a constraint for an overlapping
            component in Android Studio 3.4.](/static/studio/images/releases/constraint-target-overlay-3.4.gif)

Creating a constraint for an overlapping component in
Android Studio 3.4.

![An animation showing how to create a constraint for an overlapping
            component in Android Studio 3.5.](/static/studio/images/releases/constraint-target-overlay-3.5.gif)
...
* The **Constraint Widget** and default margin drop-down now
  allow you to use dimension resources for margins.
* In the Layout Editor toolbar, the list of devices that determine the size of
  the design surface has been updated. In addition, snapping behavior while
  resizing has been improved, and the resizing handles on the design surface are
  now always visible. When resizing, new overlays appear that show common device
  sizes.
* The Layout Editor has a new color scheme that improves consistency and reduces
  contrast between components, text, and constraints.
* Blueprint mode now includes text support for some components where text wasn't
  being shown.
...
In addition to adding [incremental annotation processing support](#incremental_ap)
...
![An animation showing code editor performance on Android Studio
            3.4.](/static/studio/images/releases/xml-editing-latency-3.4.gif)

Code editor performance on Android Studio 3.4.

![An animation showing code editor performance on Android Studio
            3.5.](/static/studio/images/releases/xml-editing-latency-3.5.gif)
...
To learn more, see [Change the build variant](/studio/run#changing-variant).

![Build Variants panel showing single variant selection by ABI.](/static/studio/images/releases/single-variant-ABI.png)
...
To learn more, see[Install and configure the NDK, CMake, and LLDB](/studio/projects/install-ndk).
...
about in the [system requirements](/studio#Requirements). To get
started, [download Android Studio](/studio) on your compatible
ChromeOS device and follow the
[installation instructions](/studio/install#chrome-os).
...
[download the module on demand](/guide/app-bundle/playcore) using the Play Core
Library. To learn more, read
[Configure conditional delivery](/studio/projects/dynamic-delivery/conditional-delivery).
...
[release notes](/studio/releases/gradle-plugin).
