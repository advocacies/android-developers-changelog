...
[Download the Android CLI](/tools/agents)

### Try Android CLI to update dependencies

Try [Android CLI](/tools/agents) if you're not using Android Studio
or prefer to do things from the command line.  
  
For example, use the
[`android studio version-lookup`](/tools/agents/android-cli#studio-version-lookup) command when you
