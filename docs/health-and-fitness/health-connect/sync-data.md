---
title: Synchronize data  |  Android health & fitness  |  Android Developers
...
# Synchronize data Stay organized with collections Save and categorize content based on your preferences.
...
2. For each entry, create a `Record` object appropriate for that data type.
   For example, create a `WeightRecord` object for data related to weight.
3. Specify a `Metadata` object with each `Record`. This includes
   ...
   ```
4. [Upsert](/guide/health-and-fitness/health-connect/develop/write-data#upsert-through-client-id) data to Health Connect using
   [`insertRecords`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#insertRecords(kotlin.collections.List)). Upserting data means that any existing
...
practices for [Write data](/guide/health-and-fitness/health-connect/plan/best-practices#write-data).
...
The [`insertRecords`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#insertRecords(kotlin.collections.List)) function returns a
[`InsertRecordsResponse`](/reference/kotlin/androidx/health/connect/client/response/InsertRecordsResponse) that contains the list of `id` values.
...
**Note:** If your app needs to read data from Health Connect, you must store the
`id`. This is necessary to correctly process deletion changelogs, as
`DeletionChange` notifications only provide the record `id`.
...
To obtain a *Changes* token, call [`getChangesToken`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#getChangesToken(androidx.health.connect.client.request.ChangesTokenRequest)) and
...
**Note:** We recommend getting separate tokens per data type instead of getting them
in bulk to avoid having an `Exception` in case one of the permissions is
revoked.
...
1. Call [`getChanges`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#getChanges(kotlin.String)) using the token to obtain a list of
   *Changes*.
2. Check each change whether its type of change is an
   [`UpsertionChange`](/reference/kotlin/androidx/health/connect/client/changes/UpsertionChange) or a [`DeletionChange`](/reference/kotlin/androidx/health/connect/client/changes/DeletionChange), and
   perform the necessary operations.
   * For `UpsertionChange`, only take changes that didn't come from the
     calling app to make sure you're not re-importing data.
...
practices for [Sync data](/guide/health-and-fitness/health-connect/plan/best-practices#sync-data).

### Process data changes

Reflect the changes to your app's datastore. For `UpsertionChange`, use the `id`
and the `lastModifiedTime` from its `metadata` to [upsert](/guide/health-and-fitness/health-connect/develop/write-data#upsert-through-client-id) the record.
For `DeletionChange`, use the `id` provided to [delete](/guide/health-and-fitness/health-connect/develop/delete-data) the record.
This requires that you have stored the record `id` as mentioned in
[Store Health Connect IDs](#store-ids).

**Note:** [`DeletionChange`](/reference/kotlin/androidx/health/connect/client/changes/DeletionChange) only contains the `id` of the deleted
record, and not the record type, due to privacy. With that, you can either
specify only 1 data type for each call of `getChanges`, or make sure that you've
stored this information separately beforehand.

## Delete data from Health Connect

When a user deletes their own data from your app, make sure that the data is
also [removed](/guide/health-and-fitness/health-connect/develop/delete-data) from Health Connect. Use [`deleteRecords`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#deleteRecords(kotlin.reflect.KClass,kotlin.collections.List,kotlin.collections.List))
...
alternative [`deleteRecords`](/reference/kotlin/androidx/health/connect/client/HealthConnectClient#deleteRecords(kotlin.reflect.KClass,androidx.health.connect.client.time.TimeRangeFilter)) that takes in a `timeRangeFilter`
is also available.

## Low-latency synchronization from wearables

To sync data from a wearable fitness device to Health Connect with low latency,
use [`CompanionDeviceService`](/reference/android/companion/CompanionDeviceService). This approach works for
...
[Bluetooth Low Energy overview](/develop/connectivity/bluetooth/ble/background#stay-connected).
...
[`CompanionDeviceManager`](/reference/android/companion/CompanionDeviceManager). This grants your app the
necessary permissions to interact with the device. For more information,
see [Companion device pairing](/develop/connectivity/bluetooth/companion-device-pairing).
...
* Search your app datastore for the most recently consumed record that also
  has an `id` from Health Connect.
* Request records from Health Connect that begin with a specific
  timestamp, and then insert or update them in your app's datastore.
...
* **Read and dedupe all data**. This is the most ideal strategy.
  ...
  + On token expiry, re-read all data from the most recent timestamp or for
    the last 30 days. Then, dedupe it against the previously read data using
    identifiers.
...
* **Delete then read data for the last 30 days**. This aligns more closely
  ...
  + Delete all data read by the app from Health Connect for the last 30
    days.
...
* **Read data for last 30 days without deduping**. This is the least ideal
  ...
  + Delete all data read by the app from Health Connect for the
    last 30 days.
...
[`Background Read`](/health-and-fitness/guides/health-connect/develop/read-data#background-read-example) permission, your user can grant your app
...
* Each time your app becomes active in the foreground. In this case, use
  lifecycle events.
* Periodically, while your app remains in the foreground. Notify users when
  new data is available, allowing them to update their screen to reflect the
  changes.
...
Delete data](/health-and-fitness/health-connect/delete-data)

[Next

Metadata requirements

arrow\_forward](/health-and-fitness/health-connect/metadata)