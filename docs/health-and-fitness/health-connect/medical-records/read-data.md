---
title: Read medical data  |  Android health & fitness  |  Android Developers
...
# Read medical data Stay organized with collections Save and categorize content based on your preferences.
...
3. Read data, such as [`MedicalResource`](/reference/kotlin/androidx/health/connect/client/records/MedicalResource) records or
   [`MedicalDataSource`](/reference/kotlin/androidx/health/connect/client/records/MedicalDataSource) details.

## Feature availability

To determine whether a user's device supports Medical Records on Health
Connect, check the availability of `FEATURE_PERSONAL_HEALTH_RECORD` on the client:
...
You can read any medical resource ([`MedicalResource`](/reference/kotlin/androidx/health/connect/client/records/MedicalResource))
...
paged requests and be mindful of [rate limiting](/health-and-fitness/guides/health-connect/plan/rate-limiting).
...
You can read any data source ([`MedicalDataSource`](/reference/kotlin/androidx/health/connect/client/records/MedicalDataSource))
written to Health Connect for the user, including those added by other apps.

A `MedicalDataSource` represents the source of medical records, such as a
hospital or clinic. This helps you identify where specific medical records
originated, which is useful for filtering data or displaying source
...
Write medical data](/health-and-fitness/health-connect/medical-records/write-data)