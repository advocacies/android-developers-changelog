---
title: Test top use cases for Health Connect  |  Android health & fitness  |  Android Developers
...
# Test top use cases for Health Connect Stay organized with collections Save and categorize content based on your preferences.
...
| **Reference** | [New Health Connect users](/guide/health-and-fitness/health-connect/design/permissions-and-data#new-health-connect-users) |

### Steps

1. Open your app.
2. Go to the promo card, modal, **Settings** screen, or similar screens that
   allow users to integrate with Health Connect for the first time.
3. Open the onboarding screen by following the steps indicated in your app.
4. On the onboarding screen, tap **Get started**.
5. On the rational screen, toggle **Allow all** to enable permissions listed
   for your app.
6. Tap **Allow** to grant permissions.

### Expected results

![Onboarding flow](/static/health-and-fitness/health-connect/test/images/hc-01.png)

### Pass and fail conditions

PASS if *all* of the following conditions are met:

* Users is taken to the rationale screen with all required read and write
  permissions.
* Granted permissions is reflected properly in the Health Connect app.

FAIL for *any* of the following reasons:

* The user is not taken to the rationale page with all required read and write
  permissions.
...
1. Open your app.
2. Navigate to the app's **Settings** screen (or similar screens) where it has
   the option to integrate with Health Connect.
3. Choose to install Health Connect.

### Expected results

![Attempt to integrate with Health Connect while uninstalled](/static/health-and-fitness/health-connect/test/images/hc-02-01.png)

### Pass and fail conditions

PASS if *all* of the following conditions are met:

* The app's **Settings** screen (or similar screens) must have a feature to
  integrate with Health Connect.
* Users must be taken to the Health Connect page in the Google Play store.

FAIL for *any* of the following reasons:

* There's no way to integrate with Health Connect in the app's **Settings**
  screen or any similar screens.
...
![Integrate with Health Connect through your app](/static/health-and-fitness/health-connect/test/images/hc-02-02.png)
...
* The app's **Settings** screen (or similar screens) must have a feature to
  integrate with Health Connect.
* Tapping the option to integrate with Health Connect must lead you to the
  Health Connect app.

FAIL for *any* of the following reasons:

* There's no feature to integrate with Health Connect in the app's **Settings**
  screen or any similar screens.
* Tapping the option to integrate with Health Connect doesn't lead you to the
  Health Connect app.
...
| Details | |
| --- | --- |
| **Description** | When a user decides to stop using Health Connect, an app must have a way to unlink from Health Connect. |

It must revoke all permissions through the app, effectively removing integration
from Health Connect.|  |  |
| --- | --- |
| **Requirements** | The Health Connect app must be installed on the phone. |
| **Reference** | [`PermissionController.revokeAllPermissions`](/reference/androidx/health/connect/client/PermissionController#revokeAllPermissions()) |

### Steps

1. Open your app.
2. Navigate to the app's **Settings** screen (or similar screens) where it has
   the option to unlink from Health Connect.
3. Choose to unlink from Health Connect.

### Expected results

![Unlink from Health Connect through your app](/static/health-and-fitness/health-connect/test/images/hc-02-03.png)
...
* The app's **Settings** screen (or similar screens) must have a feature to
  unlink from Health Connect.
* All permissions listed under your app must be revoked.

FAIL for *any* of the following reasons:

* There's no way to unlink from Health Connect in the app's **Settings** screen
  or any similar screens.
...
* The Health Connect app must be installed on the phone.
* Your app must be integrated with Health Connect.
...
| **Reference** | [Option within your settings menu](/guide/health-and-fitness/health-connect/design/permissions-and-data#option_within_your_settings_menu) |

### Steps

1. Open your app.
2. Navigate to the app's **Settings** screen (or similar screens) where it has
   the option to access the Health Connect app.
3. Choose to access or manage Health Connect.

### Expected results

![Access Health Connect from your app's settings](/static/health-and-fitness/health-connect/test/images/hc-03.png)

### Pass and fail conditions

PASS if *all* of the following conditions are met:

* The app's **Settings** screen (or similar screens) must have a way to access
  the Health Connect app.

FAIL for *any* of the following reasons:

* There's no way to access the Health Connect app in the app's **Settings**
  screen or any similar screens.
...
* The Health Connect app must be installed on the phone.
* All permissions must be granted prior to testing.

|  |  |
| --- | --- |
| **Notes** |  |

* If your app has a way to reflect denied permissions, check it from there as
  well.
* If your app can still read data from Health Connect, the app must be using a
  `dataOriginFilter` when calling a `ReadRecordsRequest`.
...
![Deny permissions through the Health Connect app](/static/health-and-fitness/health-connect/test/images/hc-04-01.png)
...
* Under **App permissions**, your app didn't appear under
  **Not allowed access** and is still under **Allowed access**.
...
* The Health Connect app must be installed on the phone.
* All permissions must be revoked prior to testing.
...
![Allow permissions through the Health Connect app](/static/health-and-fitness/health-connect/test/images/hc-04-02.png)
...
* Under **App permissions**, your app didn't appear under **Allowed access**
  and is still under **Not allowed access**.
...
| **Reference** | [Write data](/health-and-fitness/health-connect/write-data) |
...
![Write data through your app](/static/health-and-fitness/health-connect/test/images/hc-05.png)
...
* You have installed the [Health Connect Toolbox app](/health-and-fitness/health-connect/test/health-connect-toolbox).
* The **Write permission** of the required data type must be granted for the
  Health Connect Toolbox app.
* The **Read permission** of the required data type must be granted for your
  app unless you're using your app's package name for your `dataOriginFilter`.

|  |  |
| --- | --- |
| **Reference** | [Read raw data](/health-and-fitness/health-connect/read-data) |
...
![Read data from your app](/static/health-and-fitness/health-connect/test/images/hc-06.png)
...
In most apps, the data is aggregated for purposes such as displaying
...
* The **Write permission** of the required data type must be granted for the
  Health Connect Toolbox app.
* The **Read permission** of the required data type must be granted for your app
  unless you're using your app's package name for your `dataOriginFilter`.

|  |  |
| --- | --- |
| **Notes** | Aggregated data may vary depending on the values used in the `timeRangeFilter` and `dataOriginFilter`. |
| **Reference** | [Read aggregated data](/health-and-fitness/health-connect/aggregate-data) |

### Steps

1. Log multiple values for the required data type in the Health Connect Toolbox
   app.
...
![Read aggregated data from your app](/static/health-and-fitness/health-connect/test/images/hc-07.png)
...
| Details | |
| --- | --- |
| **Description** | Part of the common workflow is to update data from the Health Connect datastore. |

Updates are necessary in scenarios such as syncing and importing data.|  |  |
| --- | --- |
| **Requirements** | The **Write permission** of the required data type must be granted for your app. |
| **Reference** | [Update data](/health-and-fitness/health-connect/write-data#update-data) |
...
![Update data through your app](/static/health-and-fitness/health-connect/test/images/hc-08.png)
...
There are viewing apps that can display data coming from other source apps.
The source apps store data in Health Connect, while the viewing apps pull data
from there.|  |  |
| --- | --- |
| **Requirements** |  |
...
1. Update the values of your chosen data type using the Health Connect Toolbox
   app.
2. Check the Health Connect app to see if they are reflected.
   1. Open the Health Connect app.
   2. Choose **Data and access**.
...
![Display updated data from your app](/static/health-and-fitness/health-connect/test/images/hc-09.png)
...
* The updated values aren't reflected in either the Health Connect app or your
  app.
...
| **Requirements** | The **Write permission** of the required data type must be granted for your app. |
| **Reference** | [Delete data](/health-and-fitness/health-connect/delete-data) |
...
![Delete data through your app](/static/health-and-fitness/health-connect/test/images/hc-10.png)
