---
title: Arm Memory Tagging Extension (MTE)  |  Android NDK  |  Android Developers
...
# Arm Memory Tagging Extension (MTE) Stay organized with collections Save and categorize content based on your preferences.
...
mitigation. See
[Memory error debugging and mitigation](/ndk/guides/memory-debug) for an
...
[Developer Options](/studio/debug/dev-options) > Memory Tagging Extension in your Settings App. If this
...
store instruction with SIGSEGV (with si\_code SEGV\_MTESERR) and full information
about the memory access and the faulting address.

This mode is useful during testing as an faster alternative to [HWASan](/ndk/guides/hwasan) that
...
SEGV\_MTEAERR) without recording the faulting address or memory access.
...
so, [create your own buildType](/build/build-variants#build-types) and put the
...
**Warning:** Applications that enable MTE should be thoroughly tested on
an MTE-compatible device before being shipped to the Play Store.
Failure to do so may lead to critical bugs being discovered only
on MTE-capable devices, which may harm usability of the app.
...
**Important:** Apps built with MTE instrumentation are
**for debugging purposes only**.**Note:** MTE Stack Tagging is available starting in Android 14 QPR3.
...
can also [debug your app with Android Studio](/studio/debug) and the debugger stops at the
line causing the invalid memory access.

## Advanced Users: Using MTE in your own allocator

**Note:** if you are not aware that you are using custom allocators, you can skip
this section.
