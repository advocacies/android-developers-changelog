---
title: Neural Networks API  |  Android NDK  |  Android Developers
...
# Neural Networks API Stay organized with collections Save and categorize content based on your preferences.
...
[NNAPI Migration Guide](/ndk/guides/neuralnetworks/migration-guide).
...
* **Latency**: You don’t need to send a request over a network connection and
  ...
  that process successive frames coming from a camera.
...
* **Speed**: New hardware that is specific to neural network processing
  provides significantly faster computation than a general-purpose CPU, alone.
...
![](/static/ndk/images/nnapi/nnapi_architecture.png)
...
* **Model**: A computation graph of mathematical operations and the constant
  ...
  [`ANeuralNetworksModel`](/ndk/reference/group/neural-networks#aneuralnetworksmodel)
  instance.
* **Compilation**: Represents a configuration for compiling an NNAPI model into
  ...
  [`ANeuralNetworksCompilation`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation)
  instance.
* **Memory**: Represents shared memory, memory mapped files, and similar memory
  ...
  [`ANeuralNetworksMemory`](/ndk/reference/group/neural-networks#aneuralnetworksmemory)
  instance.
* **Execution**: Interface for applying an NNAPI model to a set of inputs and to
  ...
  [`ANeuralNetworksExecution`](/ndk/reference/group/neural-networks#aneuralnetworksexecution)
  instance.

Figure 2 shows the basic programming flow.

![](/static/ndk/images/nnapi/nnapi_flow.png)
...
[`ANeuralNetworksMemory_createFromFd()`](/ndk/reference/group/neural-networks#aneuralnetworksmemory_createfromfd)
...
You can use [native hardware buffers](/ndk/reference/group/a-hardware-buffer)
for model inputs, outputs, and constant operand values. In certain cases, an
NNAPI accelerator can access
[`AHardwareBuffer`](/ndk/reference/group/a-hardware-buffer#ahardwarebuffer)
...
listed in
[`ANeuralNetworksMemory_createFromAHardwareBuffer` reference documentation](/ndk/reference/group/neural-networks#aneuralnetworksmemory_createfromahardwarebuffer)
...
[device assignment](#device-assignment) to specify the accelerator.
...
* You can use
  `AHardwareBuffer` only for the whole buffer; you cannot use it with
  an `ARect` parameter.
* The NNAPI runtime will not flush
  the buffer. You need to make sure that the input and output buffers are
  accessible before scheduling the execution.
* There is no support for
  sync fence file descriptors.
* For an `AHardwareBuffer` with
...
[quantized](#quantized_tensors), 16-bit quantized, 32-bit integer, and 8-bit
boolean values.
...
![](/static/ndk/images/nnapi/nnapi_operands.png)
...
[More about operands](#more_operands).
...
[NNAPI API reference](/ndk/reference/neural_networks_8h) for the expected inputs
...
| Image operations | * `ANEURALNETWORKS_RESIZE_BILINEAR`* `ANEURALNETWORKS_RESIZE_NEAREST_NEIGHBOR` |
| Lookup operations | * `ANEURALNETWORKS_EMBEDDING_LOOKUP`* `ANEURALNETWORKS_HASHTABLE_LOOKUP` |
| Normalization operations | * `ANEURALNETWORKS_INSTANCE_NORMALIZATION` * `ANEURALNETWORKS_L2_NORMALIZATION`* `ANEURALNETWORKS_LOCAL_RESPONSE_NORMALIZATION` |
| Convolution operations | * `ANEURALNETWORKS_CONV_2D`* `ANEURALNETWORKS_DEPTHWISE_CONV_2D`* `ANEURALNETWORKS_GROUPED_CONV_2D`     * `ANEURALNETWORKS_TRANSPOSE_CONV_2D` |
| Pooling operations | * `ANEURALNETWORKS_AVERAGE_POOL_2D`* `ANEURALNETWORKS_L2_POOL_2D`* `ANEURALNETWORKS_MAX_POOL_2D` |
| Activation operations | * `ANEURALNETWORKS_LOG_SOFTMAX` * `ANEURALNETWORKS_LOGISTIC`* `ANEURALNETWORKS_PRELU`   * `ANEURALNETWORKS_RELU`* `ANEURALNETWORKS_RELU1`* `ANEURALNETWORKS_RELU6`* `ANEURALNETWORKS_SOFTMAX`* `ANEURALNETWORKS_TANH` |
| Other operations | * `ANEURALNETWORKS_AXIS_ALIGNED_BBOX_TRANSFORM` * `ANEURALNETWORKS_BIDIRECTIONAL_SEQUENCE_LSTM` * `ANEURALNETWORKS_BIDIRECTIONAL_SEQUENCE_RNN` * `ANEURALNETWORKS_BOX_WITH_NMS_LIMIT` * `ANEURALNETWORKS_DETECTION_POSTPROCESSING` * `ANEURALNETWORKS_FULLY_CONNECTED`* `ANEURALNETWORKS_GENERATE_PROPOSALS`   * `ANEURALNETWORKS_HEATMAP_MAX_KEYPOINT`   * `ANEURALNETWORKS_LSH_PROJECTION`* `ANEURALNETWORKS_LSTM`* `ANEURALNETWORKS_RANDOM_MULTINOMIAL`       * `ANEURALNETWORKS_QUANTIZED_16BIT_LSTM`       * `ANEURALNETWORKS_RNN`* `ANEURALNETWORKS_ROI_ALIGN`         * `ANEURALNETWORKS_ROI_POOLING`         * `ANEURALNETWORKS_SELECT`         * `ANEURALNETWORKS_SVDF`* `ANEURALNETWORKS_UNIDIRECTIONAL_SEQUENCE_LSTM`           * `ANEURALNETWORKS_UNIDIRECTIONAL_SEQUENCE_RNN` |
...
[`ANEURALNETWORKS_TENSOR_FLOAT32`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaf06d1affd33f3bc698d0c04eceb23298aee4bc05d71c31e22e39e05470e965447).
...
1. Call the
   [`ANeuralNetworksModel_create()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_create)
...
2. Add the operands to your model by calling
   [`ANeuralNetworks_addOperand()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_addoperand).
   Their data types are defined using the
   [`ANeuralNetworksOperandType`](/ndk/reference/struct/a-neural-networks-operand-type#struct_a_neural_networks_operand_type)
   data structure.
...
3. For operands that have constant values, such as weights and biases that your
   ...
   [`ANeuralNetworksModel_setOperandValue()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_setoperandvalue)
   and
   [`ANeuralNetworksModel_setOperandValueFromMemory()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_setoperandvaluefrommemory)
   functions.
...
4. For each operation in the directed graph you want to compute, add the
   ...
   [`ANeuralNetworksModel_addOperation()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_addoperation)
   function.
...
5. Identify which operands the model should treat as its inputs and outputs by
   calling the
   [`ANeuralNetworksModel_identifyInputsAndOutputs()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_identifyinputsandoutputs)
   function.
...
6. Optionally, specify whether
   ...
   [`ANeuralNetworksModel_relaxComputationFloat32toFloat16()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_relaxcomputationfloat32tofloat16).
7. Call [`ANeuralNetworksModel_finish()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_finish)
   to finalize the definition of your model. If there are no errors, this
   function returns a result code of
   [`ANEURALNETWORKS_NO_ERROR`](/ndk/reference/group/neural-networks#group___neural_networks_1ggad8097859ab1bdd06be52a8421df152d4a3d43394f34347d3a8de3c98dbd8a0365).
...
1. Call the
   [`ANeuralNetworksCompilation_create()`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_create)
   ...
   Optionally, you can use [device assignment](#device-assignment) to explicitly
...
2. You can optionally influence how the runtime trades off between battery power
   ...
   [`ANeuralNetworksCompilation_setPreference()`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_setpreference).
   ...
   * [`ANEURALNETWORKS_PREFER_LOW_POWER`](/ndk/reference/group/neural-networks#group___neural_networks_1gga034380829226e2d980b2a7e63c992f18a370c42db64448662ad79116556bcec01):
     Prefer executing in a way that minimizes battery drain. This is desirable
     for compilations that are executed often.
   * [`ANEURALNETWORKS_PREFER_FAST_SINGLE_ANSWER`](/ndk/reference/group/neural-networks#group___neural_networks_1gga034380829226e2d980b2a7e63c992f18af7fff807061a3e9358364a502691d887):
     Prefer returning a single answer as fast as possible, even if this causes
     more power consumption. This is the default.
   * [`ANEURALNETWORKS_PREFER_SUSTAINED_SPEED`](/ndk/reference/group/neural-networks#group___neural_networks_1gga034380829226e2d980b2a7e63c992f18af727c25f1e2d8dcc693c477aef4ea5f5):
...
3. You can optionally set up compilation caching by calling
   [`ANeuralNetworksCompilation_setCaching`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_setcaching).
   ...
   Use [`getCodeCacheDir()`](/reference/android/content/Context#getCodeCacheDir())
   for the `cacheDir`. The `token` specified must be unique to each model within
   the application.
4. Finalize the compilation definition by calling
   [`ANeuralNetworksCompilation_finish()`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_finish).
...
Use
[`ANeuralNetworks_getDeviceCount`](/ndk/reference/group/neural-networks#aneuralnetworks_getdevicecount)
to get the number of available devices. For each device, use
[`ANeuralNetworks_getDevice`](/ndk/reference/group/neural-networks#aneuralnetworks_getdevice)
...
* [`ANeuralNetworksDevice_getFeatureLevel`](/ndk/reference/group/neural-networks#aneuralnetworksdevice_getfeaturelevel)
* [`ANeuralNetworksDevice_getName`](/ndk/reference/group/neural-networks#aneuralnetworksdevice_getname)
* [`ANeuralNetworksDevice_getType`](/ndk/reference/group/neural-networks#aneuralnetworksdevice_gettype)
* [`ANeuralNetworksDevice_getVersion`](/ndk/reference/group/neural-networks#aneuralnetworksdevice_getversion)

##### Device assignment

Use
[`ANeuralNetworksModel_getSupportedOperationsForDevices`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_getsupportedoperationsfordevices)
...
[`ANeuralNetworksCompilation_createForDevices`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_createfordevices)
...
[`ANEURALNETWORKS_BAD_DATA`](/ndk/reference/group/neural-networks#group___neural_networks_1ggad8097859ab1bdd06be52a8421df152d4a27e8dff6d655275b20c35d80d26d202a)
is returned.
...
1. Call the
   [`ANeuralNetworksExecution_create()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_create)
...
2. Specify where your app reads the input values for the computation. Your app
   ...
   by calling
   [`ANeuralNetworksExecution_setInput()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setinput)
   or
   [`ANeuralNetworksExecution_setInputFromMemory()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setinputfrommemory)
   respectively.
   ...
   **Important:** The indexes you specify when setting
   execution inputs and outputs are indexes into the lists of inputs and outputs
   of the model as specified by
   [`ANeuralNetworksModel_identifyInputsAndOutputs()`](/ndk/reference/group/neural-networks#aneuralnetworksmodel_identifyinputsandoutputs). Don't
   ...
   [`ANeuralNetworksExecution_setInput()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setinput): one with an index of 0,
   one with an index of 1, and one with an index of 2.
3. Specify where your app writes the output values. Your app can write output values to either a
   ...
   [`ANeuralNetworksExecution_setOutput()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setoutput) or
   [`ANeuralNetworksExecution_setOutputFromMemory()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setoutputfrommemory)
   respectively.
...
4. Schedule the execution to start, by calling the
   [`ANeuralNetworksExecution_startCompute()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_startcompute)
   ...
   ```
5. Call the [`ANeuralNetworksEvent_wait()`](/ndk/reference/group/neural-networks#aneuralnetworksevent_wait)
...
[`ANeuralNetworksExecution_compute`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_compute)
...
**Note:** To estimate the impact of using `ANeuralNetworksExecution_compute` without
modifying your application, set the `debug.nn.syncexec-runtime` property to `1`,
which causes `ANeuralNetworksExecution_startCompute` to function synchronously.

#### Burst executions

On Android devices running Android 10 (API level 29) and higher, the NNAPI supports burst
executions through the
[`ANeuralNetworksBurst`](/ndk/reference/group/neural-networks#aneuralnetworksburst)
...
[`ANeuralNetworksBurst_create`](/ndk/reference/group/neural-networks#aneuralnetworksburst_create),
...
[`ANeuralNetworksExecution_burstCompute`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_burstcompute).
...
[`ANeuralNetworksBurst_free`](/ndk/reference/group/neural-networks#aneuralnetworksburst_free)
...
[`ANeuralNetworksExecution_startComputeWithDependencies()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_startcomputewithdependencies)
...
must call
[`ANeuralNetworksEvent_wait()`](/ndk/reference/group/neural-networks#group___neural_networks_1gab6569a95097d55d2bd04e789faca1a78)
...
[`ANeuralNetworksEvent_createFromSyncFenceFd()`](/ndk/reference/group/neural-networks#aneuralnetworksevent_createfromsyncfencefd),
and you can export sync fences from an event object using
[`ANeuralNetworksEvent_getSyncFenceFd()`](/ndk/reference/group/neural-networks#aneuralnetworksevent_getsyncfencefd).
...
time—use
[`ANeuralNetworksExecution_getOutputOperandRank`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_getoutputoperandrank)
and
[`ANeuralNetworksExecution_getOutputOperandDimensions`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_getoutputoperanddimensions).
...
1. Call the
   [`ANeuralNetworksMemoryDesc_create()`](/ndk/reference/group/neural-networks#aneuralnetworksmemorydesc_create)
...
2. Specify all of the intended input and output roles by calling
   [`ANeuralNetworksMemoryDesc_addInputRole()`](/ndk/reference/group/neural-networks#aneuralnetworksmemorydesc_addinputrole)
   and
   [`ANeuralNetworksMemoryDesc_addOutputRole()`](/ndk/reference/group/neural-networks#aneuralnetworksmemorydesc_addoutputrole).
...
3. Optionally, specify the memory dimensions by calling
   [`ANeuralNetworksMemoryDesc_setDimensions()`](/ndk/reference/group/neural-networks#aneuralnetworksmemorydesc_setdimensions).
...
4. Finalize the descriptor definition by calling
   [`ANeuralNetworksMemoryDesc_finish()`](/ndk/reference/group/neural-networks#aneuralnetworksmemorydesc_finish).

   ```
   ANeuralNetworksMemoryDesc_finish(desc);
   ```
5. Allocate as many memories as you need by passing the descriptor to
   [`ANeuralNetworksMemory_createFromDesc()`](/ndk/reference/group/neural-networks#aneuralnetworksmemory_createfromdesc).
...
[`ANeuralNetworksMemory_copy()`](/ndk/reference/group/neural-networks#aneuralnetworksmemory_copy).
...
stack, you can use
[`ANeuralNetworksExecution_setMeasureTiming`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setmeasuretiming)
and
[`ANeuralNetworksExecution_getDuration`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_getduration)
to get:

* execution time on an accelerator (not in the driver, which runs on the host
  processor).
...
* You can use these APIs only with an `ANeuralNetworksExecution` that was
  created with `ANeuralNetworksCompilation_createForDevices` with `numDevices =
  1`.
...
* `IPC`: The inter process communication between NNAPI Runtime and the Driver
  code
* `Driver`: the accelerator driver process.

#### Generate the profiling analysys data

Assuming you checked out the AOSP source tree at $ANDROID\_BUILD\_TOP, and
...
1. After the systrace collector is started, start your app and run your
   benchmark test.
...
#### Add statistics for your application code to systrace\_parser output

The parse\_systrace application is based on the built-in Android systrace
...
additional NNAPI [result codes](/ndk/reference/group/neural-networks#resultcode)
that enable applications to understand failures such as missed execution
deadlines.

**Note:** The APIs for setting priority and timeouts act as hints to acceleration
hardware and do not provide hard guarantees about execution time.

### Set the priority of a workload

To set the priority of an NNAPI workload, call
[`ANeuralNetworksCompilation_setPriority()`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_setpriority)
...
* To set the compilation timeout, call
  [`ANeuralNetworksCompilation_setTimeout()`](/ndk/reference/group/neural-networks#aneuralnetworkscompilation_settimeout)
  prior to calling `ANeuralNetworksCompilation_finish()`.
* To set the inference timeout, call
  [`ANeuralNetworksExecution_setTimeout()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_settimeout)
  prior to [starting the compilation](#compiling).
...
* Your application is smaller, as the trained weights take a quarter of the size
  of 32-bit tensors.
* Computations can often be executed faster. This is due to the smaller amount
  of data that needs to be fetched from memory and the efficiency of processors
  such as DSPs in doing integer math.
...
[`ANeuralNetworksOperandType`](/ndk/reference/struct/a-neural-networks-operand-type)
data structure to
[`ANEURALNETWORKS_TENSOR_QUANT8_ASYMM`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaf06d1affd33f3bc698d0c04eceb23298a07984961d5c7c12f0f8c811bedd85dc3).
...
* [`ANEURALNETWORKS_TENSOR_QUANT8_SYMM_PER_CHANNEL`](/ndk/reference/struct/a-neural-networks-symm-per-channel-quant-params)
  which you can use for representing weights to
  `CONV/DEPTHWISE_CONV/TRANSPOSED_CONV` operations.
* [`ANEURALNETWORKS_TENSOR_QUANT16_ASYMM`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaf06d1affd33f3bc698d0c04eceb23298a750fa6aa30c71eb8d093ee1c44d25a12)
  which you can use for the internal state of
  [`QUANTIZED_16BIT_LSTM`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a42edb5ab6f1c74f4a756eb1ed0183a16).
* [`ANEURALNETWORKS_TENSOR_QUANT8_SYMM`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaf06d1affd33f3bc698d0c04eceb23298a7a907ab82a0229f28bd188dd37a1ef52)
  which can be an input to
  [`ANEURALNETWORKS_DEQUANTIZE`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0ad4c9300b061d9d14669bd5acdc7538e2).

### Optional operands

A few operations, like
[`ANEURALNETWORKS_LSH_PROJECTION`](/ndk/reference/group/neural-networks#group___neural_networks_1ggaabbe492c60331b13038e39d4207940e0a800cdcec5d7ba776789cb2d1ef669965),
...
[`ANeuralNetworksExecution_setInput()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setinput)
or
[`ANeuralNetworksExecution_setOutput()`](/ndk/reference/group/neural-networks#aneuralnetworksexecution_setoutput)
