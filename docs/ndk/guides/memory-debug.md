---
title: Memory error debugging and mitigation  |  Android NDK  |  Android Developers
...
# Memory error debugging and mitigation Stay organized with collections Save and categorize content based on your preferences.
...
* Use a [memory safe language](#memory-safe-languages) whenever possible to make
  memory errors impossible
* Always use [PAC/BTI](#pacbti) to mitigate ROP/JOP attacks
* Always use [GWP-ASan](#gwp-asan) for detecting rare memory errors in
  production
* Use [HWASan](#hwasan) to detect memory errors during testing
* [MTE](#mte) is available as an option on
  [select devices](/ndk/guides/arm-mte#hwsupport).
...
**Key Point:** Good mitigation requires defense in depth. Consider combining PAC/BTI
with [MTE](#mte).
...
* There are no commercially available devices with MTE enabled by default in
  2024, but Arm's documentation explains
  [how to enable MTE for testing on Pixel 8/Pixel 8 Pro](https://learn.arm.com/learning-paths/smartphones-and-mobile/mte_on_pixel8/).
* False negative rate of 1 in 16 vs HWASan's 1 in 256
* Only available for 64-bit apps
* Requires building separate libraries for targeting both MTE-enabled and non-
  MTE enabled devices
...
**Deprecated:** ASan has been superseded by the other tools on this page, so is no
longer receiving active development or support. ASan should only be used when
HWASan is not an option.
