---
title: Permission-based access control to exported components  |  Security  |  Android Developers
...
# Permission-based access control to exported components Stay organized with collections Save and categorize content based on your preferences.
...
* **Dangerous**: High-risk permissions that could allow access to sensitive user
  data, requiring explicit user approval at runtime
* **Signature**: Granted only to apps signed with the same certificate as the
  app declaring the permission, typically used for system apps or interactions
  between apps from the same developer
...
* The component performs a sensitive task for which a permission exists that the
  user has already approved;
* The component is exported;
* The component does not perform any manual (manifest or code-level) permission
  checks;
