---
title: Content resolvers  |  Security  |  Android Developers
...
# Content resolvers Stay organized with collections Save and categorize content based on your preferences.
...
According to the [documentation](/reference/android/content/ContentResolver), `ContentResolver` is a *“class that provides applications access to the content model”*. ContentResolvers expose methods to interact, fetch, or modify content provided from the following:
...
* Permission proxy attacks, where an attacker abuses a vulnerable app that’s holding a runtime permission, such as READ\_CONTACTS, to retrieve data from a system provider.
