---
title: Device capability for billions  |  Build for Billions  |  Android Developers
...
# Device capability for billions Stay organized with collections Save and categorize content based on your preferences.
...
* Defining layouts with dp ensures that the physical size of your user interface is consistent regardless of device.
  ...
  [Supporting Multiple Screens](/guide/practices/screens_support)
...
* Test to ensure that your text and graphics work well on low- and medium-density (ldpi and mdpi) screens
  because these are [common densities](/about/dashboards#Screens),
...
* For more information about supporting different densities,
  ...
  [Supporting Different Densities](/training/multiscreen/screendensities).
...
* The Material Design guidelines describe
  [metrics and keylines](https://material.io/archive/guidelines/layout/metrics-keylines.html)
...
* For more information about supporting different screen sizes,
  see the Android training on
  [Supporting Different Screen Sizes](/training/multiscreen/screensizes).
...
* Apps should build and target the most recent version of Android to ensure
  ...
  + [`targetSdkVersion`](/guide/topics/manifest/uses-sdk-element#target) should be the latest version of Android.
  ...
  + [`minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min) Sets the minimum supported Android version.
...
* Consult the [Android dashboards](/about/dashboards#Platform), the [Google Play Developer
  Console](https://play.google.com/console/) for your app, and industry research in your target markets to
...
* Ensure your app provides a consistent experience across OS versions by using the
  [Android Support Library](/topic/libraries/support-library).
...
* Some of the highlights include:
  + v4 and v7 support library: Many framework APIs for older versions of
    ...
    `Palette`.
  + [Material Design
    support library](/tools/support-library/features#material-design): APIs to support adding Material Design components
    and patterns to your apps.
  + [Multidex support library](/topic/libraries/support-library/packages#multidex): provides support for large apps that have
    more than 65K methods. This can happen if your app is using many
    libraries.
* For more information about the available support libraries, see the [Support Libraries Features](/tools/support-library/features) section of the Android Developer site.
...
* Google Play services also include useful APIs such as [`GcmNetworkManager`](https://developers.google.com/android/reference/com/google/android/gms/gcm/GcmNetworkManager), which provides much of Android 5.0's
...
* For more information about managing your app's memory, see the Android
  ...
  Your App's Memory](/topic/performance/memory).
...
* Long-running processes stay resident in memory and can slow
  ...
  (FCM)](https://firebase.google.com/docs/cloud-messaging/) and [`GcmNetworkManager`](https://developers.google.com/android/reference/com/google/android/gms/gcm/GcmNetworkManager) to avoid long running background
...
[View the Heap and Allocations with Memory Profiler](/studio/profile/memory-profiler).
...
* The app should run smoothly on devices with ≤1GB RAM.
  ...
  [Use memory efficiently](#memory) above
  and use [Android vitals](/topic/performance/vitals)
  to identify and fix bad behaviors like slow rendering and frozen frames.
* [Picture in picture](/guide/topics/ui/picture-in-picture) (PIP) might be disabled
...
* The Proportional Set Size (PSS) of the app's RAM usage
  ...
  Your RAM Usage](/studio/profile/memory-profiler) guide.
...
You can use the
[Multiple APK feature](/google/play/publishing/multiple-apks)
