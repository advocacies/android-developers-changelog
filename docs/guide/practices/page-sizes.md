---
title: Support 16 KB page sizes  |  Compatibility  |  Android Developers
...
[Read More →](https://android-developers.googleblog.com/2025/05/prepare-play-apps-for-devices-with-16kb-page-size.html)

![](/static/distribute/images/heroes/google-play-hero.svg)
...
* Your app uses any C/C++ (native) code. If your app uses the [Android
  NDK](/ndk), then your app uses native code.
...
[APK Analyzer](/studio/debug/apk-analyzer) is a tool that lets you evaluate various aspects of a built
...
2. From the menu bar, click **Build > Analyze APK...**
   ...
   ![Studio Build menu option to launch APK Analyzer](/static/images/guide/practices/studio-analyze-apk-option.png)
...
4. Look within the `lib` folder, which hosts shared object (`.so`) files if any
   ...
   ![APK Analyzer view showing that shared object files are present](/static/images/guide/practices/apk-analyzer-so-files.png)
...
Analyzer](#identify-native-code) tool to review which libraries need to be
updated or if any code changes are required.

![Studio warning notifications about alignment issues in a project](/static/images/guide/practices/studio-align-warnings.png)

[Lint in Android Studio](/studio/write/lint) also highlights native libraries that
aren't 16 KB aligned.

![Studio linter warning about a non-aligned native library](/static/images/guide/practices/studio-align-lint.png)
...
the following section. You can also [use the command-line tools directly](#alignment-use-tools).
...
3. If any `arm64-v8a` or `x86_64` shared libraries are `UNALIGNED`, you'll need
   to [update the packaging for those libraries](#update-packaging), then [recompile your
   app](#compile-16-kb-alignment) and retest by following the steps in this section.
...
1. Make sure both Android SDK Build-Tools version 35.0.0 or higher and the
   Android NDK are installed using the [SDK Manager in Android Studio](/studio/intro/update#sdk-manager) or
   [`sdkmanager`](/tools/sdkmanager) command-line tool.
...
3. In the temporary directory that you extracted your APK file to, check the
   ...
   native libraries using APK Analyzer](#identify-native-code). Run the following command on each
   shared object file:
...
4. Check the output lines to ensure that the load segments don't have values
   ...
   values, you'll need to [update the packaging for those libraries](#update-packaging), then
   [recompile your app](#compile-16-kb-alignment) and retest by following the steps in this section.
5. Next, run the `zipalign` command-line tool on your app's APK file:
   ...
   you'll need to [update the packaging for those libraries](#update-packaging), then
   [recompile your app](#compile-16-kb-alignment) and retest by following the steps in this section.
...
1. [Update the packaging of your shared libraries](#update-packaging)
2. [Compile your app using 16 KB ELF alignment](#compile-16-kb-alignment)
3. [Fix code and resolve runtime issues](#check-code)
4. [Check SDKs for 16 KB support](#check-sdks)

**Tip:** If you update your tools to the latest versions (AGP version 8.5.1 or
higher and NDK version r28 or higher) and use 16 KB-compatible prebuilt
dependencies, then your app is 16 KB compatible by default and you can
skip to the step for [fixing code and resolving runtime issues](#check-code).
...
Gradle plugin Upgrade Assistant](/build/agp-upgrade-assistant) section for details on the upgrade process.

#### AGP version 8.5 or lower

**Note:** In AGP version 8.3 to 8.5, apps are 16 KB aligned by default.
However, `bundletool` does not zipalign APKs by default. So, the app may appear
to work, but when built from a bundle in Play, it won't install.
...
**Warning:** When you use compressed shared libraries, your app takes up more space
when installed, as libraries are extracted from the APK and copied onto disk.
Your app might more frequently fail to install because this increase in disk
usage means there is less space on device. To avoid this, upgrade to AGP version
8.5.1 or higher.
...
to the [Unity guide](/games/engines/unity/unity-on-android#16-kb-page-support). If your game runs on top of [Unreal game engine](https://www.unrealengine.com),
refer to the [Unreal guide](/games/engines/unreal/unreal-on-android#16-kb-page-support). For native game engines, continue with
this guide.

**Note:** If your app doesn't extract native libraries to the file system
([`extractNativeLibs`](/guide/topics/manifest/application-element#extractNativeLibs) set to `false`), you'll likely notice a slight
increase in your app's binary size after compiling using a 16 KB ELF
alignment. Optimizations to the package manager in Android 15 negate the runtime
costs from this increase.
...
**Important:** If your app uses any [prebuilt shared libraries](/ndk/guides/prebuilts), you must also
...
**Note:** Even if your app dynamically links to the C++ standard library
(`libc++_shared.so`) from NDKs r26 and lower, some of which don't have a
16 KB aligned `libc++_shared.so`, you should still update the alignment
of all other libraries here, and you should update your code to avoid depending
on `PAGE_SIZE`. In order to test on those lower versions of the NDK on
16 KB devices, canary releases of LTS NDK versions [r23](https://ci.android.com/builds/branches/aosp-ndk-release-r23/grid) and [r25](https://ci.android.com/builds/branches/aosp-ndk-r25-release/grid)
are available on Android CI with 16 KB aligned `libc++_shared.so`
libraries.**Note:** If you can't move to the newer Android NDK, then you might be able
to update your app to statically compile the C++ standard library into your
shared library. Refer to the [C++ Library Support](/ndk/guides/cpp-support) section for details on
statically linking to the C++ standard library and be sure to read the
[Important Considerations](/ndk/guides/cpp-support#ic) section.
...
1. Set up the [Android 15 SDK](/about/versions/15/setup-sdk) or higher.
2. Set up one of the following testing environments:
   ...
   * [Enable 16 KB mode on a device using developer options](#developer-option)
   * [Use Samsung Remote Test Lab](https://developer.samsung.com/remote-test-lab/blog/en/2025/07/07/optimize-your-applications-for-16-kb-page-size-compatibility-using-samsungs-remote-test-lab)
     on 16 KB [supported devices](https://developer.samsung.com/remotetestlab/devices/129/16kb-page-size)
...
4. Run the following [`zipalign`](/tools/zipalign) command to verify that your app is
...
5. Thoroughly test your app, focusing on any areas that might be affected by
   [changing code instances that reference specific page sizes](#check-code).
...
2. In the **SDK Platforms** tab, check **Show Package Details**, then expand
   ...
   * Google APIs Experimental 16 KB Page Size Intel x86\_64 Atom System
     Image**Note:** If you're planning to [emulate a supported Google Pixel device](/about/versions/15/get#google-pixel-devices),
   you will only need the ARM 64 v8a System Image.

   ![Download 16 KB emulator system images using the SDK Manager in
     Android Studio](/static/images/guide/practices/16kb-emulator-images.png)
3. Click **Apply > OK** to download whichever system images you selected.
4. Follow the steps to [set up a virtual device for Android 15](/about/versions/15/get#on_emulator), and when
   ...
   ![Find the 16 KB emulator image in the Other Images tab](/static/images/guide/practices/16kb-other-images-tab.png)

#### Launch the emulator

After you finish setting up the Android Emulator and virtual devices, launch the
emulator [from the target device menu](/studio/run/emulator#runningapp), or [from the command line](/studio/run/emulator-commandline).
...
![Warning in page size compat mod](/static/images/guide/practices/appcompat-dialog-16kb.png)
...
![Page size compat mode setting](/static/images/guide/practices/appcompat-setting-16kb.png)
