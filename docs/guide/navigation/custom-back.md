...
When using [Navigation Compose](/guide/navigation#framework-options), you typically use
...
[`addCallback()`](/reference/androidx/activity/OnBackPressedDispatcher#addCallback(androidx.lifecycle.LifecycleOwner,androidx.activity.OnBackPressedCallback)) method which takes a [`LifecycleOwner`](/reference/androidx/lifecycle/LifecycleOwner). This way the
...
You can provide multiple callbacks using [`addCallback()`](/reference/androidx/activity/OnBackPressedDispatcher#addCallback(androidx.activity.OnBackPressedCallback)).
...
was not enabled, and this pattern continues for any additional callbacks.

Note that when added using [`addCallback()`](/reference/androidx/activity/OnBackPressedDispatcher#addCallback(androidx.lifecycle.LifecycleOwner,androidx.activity.OnBackPressedCallback)),
...
Changing the enabled state on the callback is strongly
recommended for temporary changes as it maintains the ordering described
earlier,
which is particularly important if you have callbacks registered on multiple
different nested lifecycle owners.

However, in cases where you want to remove the callback entirely,
you should call
