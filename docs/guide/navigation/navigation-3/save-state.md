...
[`rememberNavBackStack`](/reference/kotlin/androidx/navigation3/runtime/rememberNavBackStack.composable).
...
* **Have the `@Serializable` annotation**: In addition to implementing `NavKey`,
  your key classes and objects must be marked with the [`@Serializable`](https://kotlinlang.org/api/kotlinx.serialization/kotlinx-serialization-core/kotlinx.serialization/-serializable/)
  annotation.
...
#### Remember a back stack with subtypes of `NavKey`

The `rememberNavBackStack` composable function returns a `NavBackStack<NavKey>`.
If your app defines its own subtype of `NavKey` from which all of its keys
inherit, you can preserve that typing by implementing a custom remember
function, as follows:
...
For more examples—including how to handle open polymorphism—see
[`NavBackStackSamples`](https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:navigation3/navigation3-runtime/samples/src/main/kotlin/androidx/navigation3/runtime/samples/NavBackStackSamples.kt).
...
2. Add the default [`rememberSaveableStateHolderNavEntryDecorator()`](/reference/kotlin/androidx/navigation3/runtime/rememberSaveableStateHolderNavEntryDecorator.composable)
   to the list of `entryDecorators` when constructing a `NavDisplay`.
3. Add [`rememberViewModelStoreNavEntryDecorator()`](/reference/kotlin/androidx/lifecycle/viewmodel/navigation3/rememberViewModelStoreNavEntryDecorator.composable) to the list of `entryDecorators`.
