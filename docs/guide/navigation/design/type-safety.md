...
#### Access arguments in a ViewModel

To access arguments from a type-safe route in a `ViewModel`, you can retrieve
the route from the [`SavedStateHandle`](/topic/libraries/architecture/viewmodel/viewmodel-savedstate#savedstatehandle) by calling
[`SavedStateHandle.toRoute<T>()`](/reference/kotlin/androidx/navigation/package-summary#(androidx.lifecycle.SavedStateHandle).toRoute(kotlin.collections.Map)), where `T` is your route class:
