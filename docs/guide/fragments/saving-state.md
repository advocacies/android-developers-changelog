...
to ensure that any data in your fragment is saved and restored as well.

The following table outlines the operations that cause your fragment to lose
...
The following sections describe how to properly manage the state of your data
...
A view needs an ID to retain its state. This ID must be unique within the
...
As mentioned in table 1, note that the variables are retained when the
fragment is placed on the backstack. Treating them as saved state ensures
they persist through all destructive operations.
