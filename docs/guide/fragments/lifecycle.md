---
title: Fragment lifecycle  |  App architecture  |  Android Developers
...
# Fragment lifecycle Stay organized with collections Save and categorize content based on your preferences.





Each [`Fragment`](/reference/androidx/fragment/app/Fragment) instance has
...
[`LifecycleOwner`](/reference/androidx/lifecycle/LifecycleOwner), exposing
a [`Lifecycle`](/reference/androidx/lifecycle/Lifecycle) object that
you can access through the
[`getLifecycle()`](/reference/androidx/lifecycle/LifecycleOwner#getLifecycle())
method.

Each possible `Lifecycle` state is represented in the
[`Lifecycle.State`](/reference/androidx/lifecycle/Lifecycle.State) enum.

* [`INITIALIZED`](/reference/androidx/lifecycle/Lifecycle.State#INITIALIZED)
* [`CREATED`](/reference/androidx/lifecycle/Lifecycle.State#CREATED)
...
[`LifecycleObserver`](/reference/androidx/lifecycle/LifecycleObserver), the
...
[`onStart()`](/reference/androidx/fragment/app/Fragment#onStart()),
[`onResume()`](/reference/androidx/fragment/app/Fragment#onResume()),
[`onPause()`](/reference/androidx/fragment/app/Fragment#onPause()),
[`onStop()`](/reference/androidx/fragment/app/Fragment#onStop()), and
[`onDestroy()`](/reference/androidx/fragment/app/Fragment#onDestroy()).
...
[`LifecycleOwner`](/reference/androidx/lifecycle/LifecycleOwner)
for their view, which can be accessed using
[`getViewLifecycleOwner()`](/reference/androidx/fragment/app/Fragment#getViewLifecycleOwner())
or
[`getViewLifecycleOwnerLiveData()`](/reference/androidx/fragment/app/Fragment#getViewLifecycleOwnerLiveData()).
...
[`LiveData`](/topic/libraries/architecture/livedata) that is only meant
...
must be added to a
[`FragmentManager`](/reference/androidx/fragment/app/FragmentManager). The
...
[`findFragmentById()`](/reference/androidx/fragment/app/FragmentManager#findFragmentById(int))
return this fragment.

`onAttach()` is always called before any [Lifecycle state changes](#states).
...
`onDetach()` is always called after any [Lifecycle state changes](#states).

Note that these callbacks are unrelated to the
[`FragmentTransaction`](/reference/androidx/fragment/app/FragmentTransaction)
methods
[`attach()`](/reference/androidx/fragment/app/FragmentTransaction#attach(androidx.fragment.app.Fragment))
and
[`detach()`](/reference/androidx/fragment/app/FragmentTransaction#detach(androidx.fragment.app.Fragment)).
For more information on these methods, see
[Fragment transactions](/guide/fragments/transactions#attach-detach).

**Caution:** Avoid reusing `Fragment` instances after they are removed from the
...
* As part of a `FragmentTransaction`, you can set a maximum lifecycle state
  on a fragment using
  [`setMaxLifecycle()`](/reference/androidx/fragment/app/FragmentTransaction#setMaxLifecycle(androidx.fragment.app.Fragment,%20androidx.lifecycle.Lifecycle.State)) .
...
**Caution:** Avoid using the `<fragment>` tag to add a fragment using XML, as
the `<fragment>` tag allows a fragment to move beyond the state of its
`FragmentManager`. Instead, always use
[`FragmentContainerView`](/reference/androidx/fragment/app/FragmentContainerView)
for adding a fragment using XML.

![fragment lifecycle states and their relation both the fragment's
            lifecycle callbacks and the fragment's view lifecycle](/static/images/guide/fragments/fragment-view-lifecycle.png)
...
[`Lifecycle.Event`](/reference/androidx/lifecycle/Lifecycle.Event) is
...
[`onAttach()`](/reference/androidx/fragment/app/Fragment#onAttach(android.content.Context))
...
[`SavedStateRegistry`](/reference/androidx/savedstate/SavedStateRegistry).
...
[`Bundle`](/reference/android/os/Bundle) argument containing any state
previously saved by
[`onSaveInstanceState()`](/reference/androidx/fragment/app/Fragment#onSaveInstanceState(android.os.Bundle)).
...
[Saving state with fragments](/guide/fragments/saving-state) for more
details.

#### Fragment CREATED and View INITIALIZED

The fragment's view `Lifecycle` is created only when your `Fragment`
provides a valid [`View`](/reference/android/view/View) instance. In
most cases, you can use the
[fragment constructors](/reference/androidx/fragment/app/Fragment#Fragment(int))
...
[`getView()`](/reference/androidx/fragment/app/Fragment#getView()). The
[`getViewLifecycleOwnerLiveData()`](/reference/androidx/fragment/app/Fragment#getViewLifecycleOwnerLiveData())
...
to start observing [`LiveData`](/topic/libraries/architecture/livedata)
instances whose callbacks update the fragment's view, and to set up
adapters on any
[`RecyclerView`](/reference/androidx/recyclerview/widget/RecyclerView) or
[`ViewPager2`](/reference/androidx/viewpager2/widget/ViewPager2) instances
...
[`ON_CREATE`](/reference/androidx/lifecycle/Lifecycle.Event#ON_CREATE) event
...
[`onViewStateRestored()`](/reference/androidx/fragment/app/Fragment#onViewStateRestored(android.os.Bundle))
callback.

#### Fragment and View STARTED

It is strongly recommended to tie
[Lifecycle-aware components](/topic/libraries/architecture/lifecycle) to
...
[`onStart()`](/reference/androidx/fragment/app/Fragment#onStart()) callback
is invoked.

**Note:** Components such as
[`ViewPager2`](/reference/androidx/viewpager2/widget/ViewPager2) set the
maximum `Lifecycle` of offscreen fragments to `STARTED`.

#### Fragment and View RESUMED

When the fragment is visible, all
[`Animator`](/reference/android/animation/Animator) and
[`Transition`](/reference/androidx/transition/Transition) effects have
...
to [handle input method visibility](/training/keyboard-input/visibility).

### Downward state transitions

When a fragment moves downward to a lower lifecycle state, the
relevant [`Lifecycle.Event`](/reference/androidx/lifecycle/Lifecycle.Event)
...
visible, the `Lifecycle`s for the fragment and for its view are moved back
to the `STARTED` state and emit the
[`ON_PAUSE`](/reference/androidx/lifecycle/Lifecycle.Event#ON_PAUSE) event
to their observers. The fragment then invokes its
[`onPause()`](/reference/androidx/fragment/app/Fragment#onPause()) callback.

#### Fragment and View CREATED

Once the fragment is no longer visible, the `Lifecycle`s for the fragment
and for its view are moved into the `CREATED` state and emit the
[`ON_STOP`](/reference/androidx/lifecycle/Lifecycle.Event#ON_STOP) event
...
[`onStop()`](/reference/androidx/fragment/app/Fragment#onStop()) callback
...
before [`onStop()`](/reference/androidx/fragment/app/Fragment#onStop()).
For API levels 28 and higher, the calling order is reversed.

![calling order differences for onStop() and onSaveInstanceState()](/static/images/guide/fragments/stop-save-order.png)
...
[animations and transitions](/training/basics/fragments/animate) have
...
the [`ON_DESTROY`](/reference/androidx/lifecycle/Lifecycle.Event#ON_DESTROY)
...
[`ON_DESTROY`](/reference/androidx/lifecycle/Lifecycle.Event#ON_DESTROY)
event to its observers. The fragment then invokes its
[`onDestroy()`](/reference/androidx/fragment/app/Fragment#onDestroy())
