---
title: Create a cloud media provider for Android  |  App data and files  |  Android Developers
...
# Create a cloud media provider for Android Stay organized with collections Save and categorize content based on your preferences.
...
photo storage solution, you should consider creating a [document provider](/guide/topics/providers/create-document-provider)
instead.
...
* If there is only one eligible cloud provider on the device, that app will be
  selected as the current provider automatically.
...
![Sequence diagram showing flow from photo picker to a cloud media provider](/static/guide/topics/providers/images/cloud-media-provider-sequence.svg)


**Figure 1:**
Event sequence diagram during a photo selection session.

1. The system initializes the user's preferred cloud provider and periodically
   syncs media metadata into the Android photo picker backend.
2. When an Android app launches the photo picker, before showing a merged local
   or cloud item grid to the user, the photo picker performs a latency-sensitive
   incremental sync with the cloud provider to ensure results are as up-to-date
   as possible. After receiving a response, or when the deadline is reached, the
   photo picker grid now displays all accessible photos, combining those stored
   locally on your device with those synced from the cloud.
3. While the user scrolls, the photo picker fetches media thumbnails from the
   cloud media provider to display in the UI.
4. When the user completes the session and the results include a cloud media
   item, the photo picker requests file descriptors for the content, generates a
   URI, and grants access to the file to the calling application.
5. The app is now able to open the URI and has read-only access to the media
   contents. By default, sensitive metadata is redacted. The photo picker
   leverages the FUSE file system to coordinate data exchange between the
   Android app and the cloud media provider.
...
Derived from `android.content.ContentProvider`, the [`CloudMediaProvider`](/reference/kotlin/android/provider/CloudMediaProvider)
...
Android photo picker incorporates a [`CloudMediaProviderContract`](/reference/kotlin/android/provider/CloudMediaProviderContract) class.
...
The [`onGetMediaCollectionInfo()`](/reference/kotlin/android/provider/CloudMediaProvider#ongetmediacollectioninfo) method is used by the operating system to
...
The returned [`MediaCollectionInfo`](/reference/kotlin/android/provider/CloudMediaProviderContract.MediaCollectionInfo) bundle includes the following constants:

* [`MEDIA_COLLECTION_ID`](/reference/kotlin/android/provider/CloudMediaProviderContract.MediaCollectionInfo#MEDIA_COLLECTION_ID:kotlin.String)
* [`LAST_MEDIA_SYNC_GENERATION`](/reference/kotlin/android/provider/CloudMediaProviderContract.MediaCollectionInfo#LAST_MEDIA_SYNC_GENERATION:kotlin.String)
* [`ACCOUNT_NAME`](/reference/kotlin/android/provider/CloudMediaProviderContract.MediaCollectionInfo#ACCOUNT_NAME:kotlin.String)
* [`ACCOUNT_CONFIGURATION_INTENT`](/reference/kotlin/android/provider/CloudMediaProviderContract.MediaCollectionInfo#ACCOUNT_CONFIGURATION_INTENT:kotlin.String)

### onQueryMedia

The [`onQueryMedia()`](/reference/kotlin/android/provider/CloudMediaProvider#onquerymedia) method is used to populate the main photo grid in
...
The returned [`CloudMediaProviderContract`](/reference/kotlin/android/provider/CloudMediaProviderContract) bundle includes the following
constants:

* [`EXTRA_ALBUM_ID`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_album_id)
* [`EXTRA_LOOPING_PLAYBACK_ENABLED`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_looping_playback_enabled)
* [`EXTRA_MEDIA_COLLECTION_ID`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_media_collection_id)
* [`EXTRA_PAGE_SIZE`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_page_size)
* [`EXTRA_PAGE_TOKEN`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_page_token)
* [`EXTRA_PREVIEW_THUMBNAIL`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_preview_thumbnail)
* [`EXTRA_SURFACE_CONTROLLER_AUDIO_MUTE_ENABLED`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_surface_controller_audio_mute_enabled)
* [`EXTRA_SYNC_GENERATION`](/reference/kotlin/android/provider/CloudMediaProviderContract#extra_sync_generation)
* [`MANAGE_CLOUD_MEDIA_PROVIDERS_PERMISSION`](/reference/kotlin/android/provider/CloudMediaProviderContract#manage_cloud_media_providers_permission)
* [`PROVIDER_INTERFACE`](/reference/kotlin/android/provider/CloudMediaProviderContract#provider_interface)
...
The [`onQueryDeletedMedia()`](/reference/kotlin/android/provider/CloudMediaProvider#onquerydeletedmedia) method is used to ensure deleted items in the
...
The [`onQueryAlbums()`](/reference/kotlin/android/provider/CloudMediaProvider#onqueryalbums) method is used to fetch a list of Cloud albums that
are available in the cloud provider, and their associated metadata. See
[`CloudMediaProviderContract.AlbumColumns`](/reference/kotlin/android/provider/CloudMediaProviderContract.AlbumColumns) for additional details.
...
The [`onOpenMedia()`](/reference/kotlin/android/provider/CloudMediaProvider#onopenmedia) method should return the full size media identified by
...
The [`onOpenPreview()`](/reference/kotlin/android/provider/CloudMediaProvider#onopenpreview) method should return a thumbnail of the provided
...
The [`onCreateCloudMediaSurfaceController()`](/reference/kotlin/android/provider/CloudMediaProvider#oncreatecloudmediasurfacecontroller) method should return a
...
* [`onConfigChange`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onconfigchange)
* [`onDestroy`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onDestroy)
* [`onMediaPause`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onMediaPause)
* [`onMediaPlay`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onMediaPlay)
* [`onMediaSeekTo`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onMediaSeekTo)
* [`onPlayerCreate`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onPlayerCreate)
* [`onPlayerRelease`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onPlayerRelease)
* [`onSurfaceChanged`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onSurfaceChanged)
* [`onSurfaceCreated`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onSurfaceCreated)
* [`onSurfaceDestroyed`](/reference/kotlin/android/provider/CloudMediaProvider.CloudMediaSurfaceController#onSurfaceDestroyed)

[Previous

arrow\_back

Create a custom document provider](/guide/topics/providers/create-document-provider)