---
title: Make apps more accessible  |  App quality  |  Android Developers
...
# Make apps more accessible Stay organized with collections Save and categorize content based on your preferences.
...
on this page at [Make apps more accessible (Views)](/guide/topics/ui/accessibility/views/apps-views).
...
* If the text is smaller than 18sp, or if the text is bold and smaller than
  ...
  [color contrast ratio](https://m3.material.io/foundations/designing/color-contrast) of at least 4.5:1.
...
![Two examples of the word 'Text' on colored backgrounds. The example on the left has low color contrast between the text and background, while the example on right has sufficient color contrast.](/static/images/guide/topics/ui/accessibility/color-contrast.svg)


**Figure 1.** Lower than recommended (left) and sufficient (right) color
contrast.
...
**Note:** For precise input (mouses and trackpads), the touch target can be smaller.
See [Pointer interactions](/design/ui/desktop/guides/interaction/pointer-interactions) for more information.
...
[Minimum touch target sizes](/develop/ui/compose/accessibility/api-defaults#minimum-target-sizes).
...
* Use descriptions to convey the purpose and result of the interaction, not the
  visual details. Use the [`Role` semantics property](/guide/topics/ui/accessibility/composables#describe)
...
* Use the `hideFromAccessibility` API to mark purely decorative elements so
  ...
  For more elaborate use cases, see [Merging and clearing](/develop/ui/compose/accessibility/merging-clearing).
* Test your code to make sure the content description is delivered as expected.
  Android Lint, Compose testing, and [manual and automated test tools](/develop/ui/compose/accessibility/testing)
...
* [Accessibility in Jetpack Compose](/codelabs/jetpack-compose-accessibility#0)
...
* [Make apps more accessible (Views)](/guide/topics/ui/accessibility/views/apps-views)