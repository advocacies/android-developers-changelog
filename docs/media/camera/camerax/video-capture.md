---
title: CameraX video capturing architecture  |  Android media  |  Android Developers
...
# CameraX video capturing architecture Stay organized with collections Save and categorize content based on your preferences.
...
![conceptual diagram for a video and audio capturing system](/static/images/training/camera/camerax/conceptual-capturing-system.png)


**Figure 1.** Conceptual diagram for a video and audio
capturing system.

In CameraX, the solution for video capturing is the
[`VideoCapture`](/reference/androidx/camera/video/VideoCapture)
use case:

![conceptual diagram that shows how camera x handles the
         video capture use case](/static/images/training/camera/camerax/videocapture-use-case.png)


**Figure 2.** Conceptual diagram that shows how CameraX
handles the `VideoCapture` use case.
...
**Note:** `VideoCapture` is implemented in the `camera-video` library inside the
CameraX package, available in versions `1.1.0-alpha10` and higher.

The VideoCapture API consists of the following objects that communicate
with applications:

* [`VideoCapture`](/reference/androidx/camera/video/VideoCapture) is the
  top-level use case class. `VideoCapture` binds to
  a `LifecycleOwner` with a `CameraSelector` and other CameraX
  UseCases. For more information about these concepts and usages, see
  [CameraX Architecture](/training/camerax/architecture).
* A [`Recorder`](/reference/androidx/camera/video/Recorder) is an
  implementation of VideoOutput that is tightly coupled with `VideoCapture`.
  `Recorder` is used to perform the video and audio capturing. An
  application **creates** recordings from a `Recorder`.
* A [`PendingRecording`](/reference/androidx/camera/video/PendingRecording)
  configures a recording, providing options like enabling audio and setting
  an event listener. You must use a `Recorder` to create a `PendingRecording`.
  A `PendingRecording` does not record anything.
* A [`Recording`](/reference/androidx/camera/video/Recording) performs the
  actual recording. You must use a `PendingRecording` to create a `Recording`.

Figure 3 shows the relationships between these objects:

![diagram showing the interactions that occur in a video
         capture use case](/static/images/training/camera/camerax/videocapture-roles-interaction.png)


**Figure 3.** Diagram showing the interactions that occur
in a VideoCapture use case.

**Legend:**

1. Create a [`Recorder`](/reference/androidx/camera/video/Recorder) with
   [`QualitySelector`](/reference/androidx/camera/video/QualitySelector).
2. Configure the `Recorder` with one of the
   [`OutputOptions`](/reference/androidx/camera/video/OutputOptions).
3. Enable audio with
   [`withAudioEnabled()`](/reference/androidx/camera/video/PendingRecording#withAudioEnabled())
   if needed.
4. Call [`start()`](/reference/androidx/camera/video/PendingRecording#start())
   with a [`VideoRecordEvent`](/reference/androidx/camera/video/VideoRecordEvent)
   listener to begin recording.
5. Use `pause()`/`resume()`/`stop()` on the
   [`Recording`](/reference/androidx/camera/video/Recording)
   to control the recording.
6. Respond to [`VideoRecordEvents`](/reference/androidx/camera/video/VideoRecordEvent)
...
CameraX `Recorder` supports the following pre-defined [`Qualities`](/reference/androidx/camera/video/Quality)
...
* Provide a few preferred resolutions by using `fromOrderedList()`, and
  ...
  capability, refer to `QualitySelector`'s [`FallbackStrategy specification`](/reference/androidx/camera/video/FallbackStrategy)
...
* Query the camera capabilities first, and choose from the supported
  ...
  [`QualitySelector.getSupportedQualities()`](/reference/androidx/camera/video/QualitySelector#getSupportedQualities(androidx.camera.core.CameraInfo))
...
Note that [`bindToLifecycle()`](/reference/androidx/camera/lifecycle/ProcessCameraProvider#bindToLifecycle(androidx.lifecycle.LifecycleOwner,%20androidx.camera.core.CameraSelector,%20androidx.camera.core.UseCase...)) returns a [`Camera`](/reference/androidx/camera/core/Camera) object. See [this guide](/training/camerax/configuration#camera-output) for more information about controlling camera output, such as zoom and exposure.

**Note:** The eventual video codec and container format are not
currently configurable.

The `Recorder` selects the most suitable format for the system. The most
common video codec is
[H.264 AVC](/reference/android/media/MediaFormat#MIMETYPE_VIDEO_AVC)) with
container format
[MPEG-4](/reference/android/media/MediaFormat#MIMETYPE_VIDEO_MPEG4).
...
3. Use `start()` to register a
   [`VideoRecordEvent`](/reference/androidx/camera/video/VideoRecordEvent)
   listener, and begin video capturing.
...
* `FileDescriptorOutputOptions` for capturing into a
  [`FileDescriptor`](/reference/java/io/FileDescriptor).
* `FileOutputOptions` for capturing into a [`File`](/reference/java/io/File).
* `MediaStoreOutputOptions` for capturing into a
  [`MediaStore`](/reference/android/provider/MediaStore).
...
* Register a listener to receive video recording events
  with `start(Executor, Consumer<VideoRecordEvent>)`.
* Allow a recording to continuously record while the VideoCapture it's attached
  to is rebound to another camera, with
...
There are three MirrorMode options: MIRROR\_MODE\_OFF, MIRROR\_MODE\_ON, and
MIRROR\_MODE\_ON\_FRONT\_ONLY. To align to the
camera preview, Google recommends using MIROR\_MODE\_ON\_FRONT\_ONLY, which means
that
...
[`MirrorMode constants`](/reference/androidx/camera/core/MirrorMode).

This code snippet shows how to call
`VideoCapture.Builder.setMirrorMode()` using `MIRROR_MODE_ON_FRONT_ONLY`. For
more information, see [`setMirrorMode()`](/reference/androidx/camera/video/VideoCapture.Builder#setMirrorMode(int)).
...
* [`pause`](/reference/androidx/camera/video/Recording#pause())
  to pause the current active recording.
* [`resume()`](/reference/androidx/camera/video/Recording#resume())
  to resume a paused active recording.
* [`stop()`](/reference/androidx/camera/video/Recording#stop())
  to finish recording and flush any associated recording objects.
* [`mute()`](/reference/androidx/camera/video/Recording#mute())
  to mute or un-mute the current recording.
...
* `VideoRecordEvent.EVENT_TYPE_STATUS` is used for recording statistics such
  as current file size and recorded time span.
* `VideoRecordEvent.EVENT_TYPE_FINALIZE` is used for the recording result
  and includes information such as the URI of the final file along with
  any related errors.
...
* [CameraX release notes](/jetpack/androidx/releases/camera)
