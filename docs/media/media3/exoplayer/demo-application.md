---
title: ExoPlayer demo application  |  Android media  |  Android Developers
url: https://developer.android.com/media/media3/exoplayer/demo-application
source: html-scrape
---
...
# ExoPlayer demo application Stay organized with collections Save and categorize content based on your preferences.
...
1. To provide a relatively simple yet fully-featured example of ExoPlayer usage.
   ...
   your own app.
2. To make it easy to try ExoPlayer. The demo app can be used to test playback
...
our [GitHub project](https://github.com/androidx/media). If you haven't already done so, clone the project into a
local directory:
...
![The project in Android Studio](/static/guide/topics/media/exoplayer/images/demo-app-project.png)
...
instead, please read the emulators section of [Supported devices](/guide/topics/media/exoplayer/supported-devices) and ensure
that your Virtual Device uses a system image with an API level of at least 23.

![SampleChooserActivity and PlayerActivity](/static/guide/topics/media/exoplayer/images/demo-app-screenshots.png)
...
1. Build each of the extensions that you want to include. Note that this is a
   ...
   instructions.
2. In Android Studio's Build Variants view, set the build variant for the demo
   module to `withDecoderExtensionsDebug` or `withDecoderExtensionsRelease` as
   shown in the following image.

   ![Selecting the demo `withDecoderExtensionsDebug` build variant](/static/guide/topics/media/exoplayer/images/demo-app-build-variants.png)
...
preferred, as described in the following sections.
...
**Note:** On Android TV devices and emulators on API level 31 and above, an
alternative action needs to be used which is
`androidx.media3.demo.main.action.BROWSE` due to limitations on such devices.
...
* Sample configuration extras:
  ...
  + `subtitle_mime_type` [String] The MIME type of subtitle\_uri (required if
    subtitle\_uri is set).
  + `subtitle_language` [String] The BCP47 language code of the subtitle file
    (ignored if subtitle\_uri is not set).
