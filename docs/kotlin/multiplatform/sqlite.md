---
title: Set up SQLite for KMP  |  Kotlin  |  Android Developers
...
# Set up SQLite for KMP Stay organized with collections Save and categorize content based on your preferences.
...
* [`SQLiteDriver`](/reference/kotlin/androidx/sqlite/SQLiteDriver) - It is the entry point to use SQLite and is responsible
  for opening database connections.
...
* Open a database connection using the instantiated `SQLiteDriver`
  implementation.
...
**Caution:** Database connections and statements are resources that need to be
managed. All prepared statements should be closed once they are no longer
needed. Similarly if a database connection is no longer needed, then it should
be closed.
...
Any usage of low-level Support SQLite API components (such as the
`SupportSQLiteDatabase` interface) needs to be migrated to the equivalent
SQLite Driver components.

### Kotlin Multiplatform

Perform a transaction using low-level `SQLiteConnection`

**Note:** It is always recommended to use `RoomDatabase` transaction APIs as
those allow nested transactions and are safer to use than the APIs available
in `androidx.sqlite`.
