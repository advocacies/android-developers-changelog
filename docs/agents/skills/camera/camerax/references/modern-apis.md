---
title: Modern Android camera API recommendations  |  Android Developers
url: https://developer.android.com/agents/skills/camera/camerax/references/modern-apis
source: html-scrape
---

# Modern Android camera API recommendations Stay organized with collections Save and categorize content based on your preferences.
...
* **`CameraSelector`**: Use `DEFAULT_BACK_CAMERA` or `DEFAULT_FRONT_CAMERA`
  instead of hardcoding camera IDs. Use `filter` if you need specific lens
  capabilities.
* **Extensions**: Before enabling advanced modes, such as night, bokeh, and face
  retouch, use `ExtensionsManager` to query whether the device supports them.
* **Foldables**: Observe `Lifecycle` and `Viewport` updates to handle posture
  changes, like a half-opened posture, on foldable devices.
