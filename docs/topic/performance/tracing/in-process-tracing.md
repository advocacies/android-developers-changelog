---
title: In-process tracing  |  App quality  |  Android Developers
...
# In-process tracing Stay organized with collections Save and categorize content based on your preferences.





The [`androidx.tracing:tracing:2.0.0-beta01`](/jetpack/androidx/releases/tracing) library is a low-overhead
...
**Warning:** The `setGlobalTracer` method is annotated with `@DelicateTracingApi`.
It should only be called once per process lifecycle, typically by the
application entry point, and never by libraries.
...
![Screen capture of a basic Perfetto trace](/static/topic/performance/images/in-process-tracing/basic.png)
...
![Screen capture of a basic Perfetto trace with nested sections](/static/topic/performance/images/in-process-tracing/basic_nested.png)
...
![Screen capture of a basic Perfetto trace with additional metadata](/static/topic/performance/images/in-process-tracing/basic_with_context.png)
...
![Screen capture of a Perfetto trace with context propagation](/static/topic/performance/images/in-process-tracing/context_propagation.png)
...
**Note:** The `traceCoroutine` API uses `PropagationToken`s under the hood that are
attached to the underlying `coroutineContext` responsible for overseeing
execution. The core `Tracer` API also makes it possible to **bring your own
implementation of context propagation** if you choose.**Note:** In `2.0.0-beta01`, context propagation uses Perfetto flows to represent
the flow of execution. This is an imperfect representation, as it presents you
with a thread-centric view. The `Tracer` therefore keeps track of when
coroutines are suspended and resumed. This is also why you might see multiple
slices that correspond to a single `suspend` function, as it suspends and
resumes.**Note:** We are improving how context propagation can be visualized in Perfetto UI,
to better represent fan-outs for example.
...
![Screen capture of a Perfetto trace with manual context propagation](/static/topic/performance/images/in-process-tracing/manual_context_propagation.png)
...
**Note:** You can also use the `Tracer.tokenForManualPropagation()` API if you want
complete control over how context propagation works. This is especially useful
when your async workflow is convoluted (such as, when you chain operators on a
Kotlin `Flow`).
...
![Opening multiple trace files in Perfetto UI](/static/topic/performance/images/in-process-tracing/multiple_trace_files.png)


**Figure 6.**
Opening multiple trace files in Perfetto UI.

**Note:** This feature requires that the [MultiTraceOpen](https://ui.perfetto.dev/#!/plugins/dev.perfetto.MultiTraceOpen) plugin be enabled
in the Perfetto UI.**Note:** This integration works automatically because the identifiers that the
in-process tracer uses for `processes` and `threads` are identical.
In addition, the clocks used for in-process and system tracing are
synchronized, so these events automatically line up.
...
![Screen capture of a Perfetto trace with correlated slices](/static/topic/performance/images/in-process-tracing/correlating_slices.png)


**Figure 7.**
Screen capture of a Perfetto trace with correlated slices.

**Note:** Note the `Arguments` section, and that Perfetto chooses a consistent color
scheme for slices when they share a `correlationId`.
...
![Screen capture of a Perfetto trace with call stack information](/static/topic/performance/images/in-process-tracing/call_stack_entry.png)
