...
# Data layer Stay organized with collections Save and categorize content based on your preferences.
...
the benefits of the data layer, check out the [Architecture Overview page](/jetpack/guide).

**Note:** The recommendations and best practices present in this page can be applied
...
![In a typical architecture, the data layer's repositories provide data
    to the rest of the app and depend on the data sources.](/static/topic/libraries/architecture/images/mad-arch-data-overview.png)
...
classes (see the [UI layer guide](/jetpack/guide/ui-layer#state-holders)) or use case classes (see the [domain layer
guide](/jetpack/guide/domain-layer)) should never have a data source as a direct dependency. Using
...
threads. See the [threading section](#threading) for more details.

Following [dependency injection](/training/dependency-injection) best practices, the repository takes data
...
forget to split functionalities if the repository needs to handle data from
another source in a later version of your app.
...
* **To be notified of data changes over time**, expose [flows](/kotlin/flow).
...
[SharedPreferences](/training/data-storage/shared-preferences) to [DataStore](/topic/libraries/architecture/datastore)) without affecting the layer that calls
that source.

**Note:** When migrating over to a new implementation of a data source, you might
create an interface for the data source and have two implementations of the data
source: one for the old backing technology, and one for the new one. In that
case, it's fine to use the name of the technology for the data source class
names (even though it's an implementation detail) because the repository only
sees the interface, not the data source classes themselves. When you finish the
migration, you can rename the new class to not contain the implementation detail
in its name.
...
![In the example, UserRepository depends on two other repository classes:
    ...
    RegistrationRepository, which depends on other registration data sources.](/static/topic/libraries/architecture/images/mad-arch-data-multiple-repos.png)


**Figure 2.** Dependency graph of a repository that depends on other
repositories.

**Note:** Traditionally, some developers have called repository classes that depend
on other repository classes *managers*—for example, `UserManager` instead of
`UserRepository`. You can use this naming convention if you prefer.
...
method calls provided by [Room](/training/data-storage/room), [Retrofit](https://square.github.io/retrofit/) or [Ktor](https://ktor.io/). Your
repository can take advantage of these APIs when they are available.

To learn more about threading, see the [guide to background processing](/guide/background). For
Kotlin users, [coroutines](/kotlin/coroutines) are the recommended option.
...
`RegistrationActivity`, or to a backstack using a [`NavEntryDecorator`](/guide/navigation/navigation-3/naventrydecorators#apply-decorators).
...
injection](/training/dependency-injection) best practices where the dependencies are managed and can be
...
* It adapts external data types to data types used by your app—for example,
  your app might use a different data type to represent dates.
* It provides better separation of concerns—for example, members of a large
  team could work individually on the network and UI layers of a feature if
  the model class is defined beforehand.
...
network request](#network-request) section for an example of a UI-oriented operation.
...
See the [Implement in-memory data caching](#in-memory-cache) section to learn more.
...
screen](#make_an_operation_live_longer_than_the_screen) section.
...
the [Schedule tasks using WorkManager](#workmanager) section to learn more.
...
a `Result<T>` type instead of `T`, making the UI aware of *known* errors that
could occur in certain scenarios.
...
[`HttpURLConnection`](/reference/java/net/HttpURLConnection). Relying on interfaces makes API implementations
swappable in your app.

**Key Point:** **Relying on interfaces makes API implementations swappable in your
app.** In addition to providing scalability and allowing you to replace
dependencies more easily, it also favors testability because you can inject fake
data source implementations in tests.
...
caching](#in-memory-cache) section.
...
[UI layer](/jetpack/guide/ui-layer) guide.
...
* For *large datasets* that need to be queried, need referential integrity, or
  need partial updates, save the data in a *Room database*. In the News app
  example, the news articles or authors could be saved in the database.
* For *small datasets* that only need to be retrieved and set (not queried or
  updated partially), use *DataStore*. In the News app example, the user's
  preferred date format or other display preferences could be saved in
  DataStore.
* For *chunks of data* like a JSON object, use a *file*.

As mentioned in the [Source of truth](#source-of-truth) section, each data source works with
...
[data access object (DAO)](/training/data-storage/room/accessing-data) or the database itself as a parameter. For
...
To learn more about working with the Room APIs, see the [Room guides](/training/data-storage/room).

#### DataStore as a data source

[DataStore](/topic/libraries/architecture/datastore) is perfect for storing key-value pairs like user settings.
...
guides](/topic/libraries/architecture/datastore).
...
To learn more about working with file storage, see the [Storage overview](/training/data-storage)
page.
...
[WorkManager](/topic/libraries/architecture/workmanager) makes it easy to schedule asynchronous and reliable work and
can take care of constraint management. It's the recommended library for
persistent work. To perform the task defined above, a [`Worker`](/topic/libraries/architecture/workmanager/advanced/coroutineworker) class is
...
the WorkManager request using the [App Startup](/topic/libraries/app-startup) library that calls the
repository from an [`Initializer`](/reference/kotlin/androidx/startup/Initializer).

To learn more about working with WorkManager APIs, see the [WorkManager
guides](/topic/libraries/architecture/workmanager).

## Testing

[Dependency injection](/training/dependency-injection) best practices help when testing your app. It's also
...
[General testing guidance](/training/testing) applies when testing the data layer. For unit
...
control in your tests. To learn more, see the [Test and debug your database](/training/data-storage/room/testing-db#android)
page.
...
* [Build an offline-first app](/topic/architecture/data-layer/offline-first)
