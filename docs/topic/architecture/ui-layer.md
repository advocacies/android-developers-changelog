...
# UI layer Stay organized with collections Save and categorize content based on your preferences.
...
![In a typical architecture, the UI layer's UI elements depend on state
    ...
    optional domain layer.](/static/topic/libraries/architecture/images/mad-arch-ui-overview.png)


**Figure 1.** The UI layer's role in app architecture.

**Note:** The recommendations and best practices on this page can be applied
to a broad spectrum of apps to allow them to scale, improve their quality and
robustness, and make them easier to test. Treat them as
...
![A sample news app showing article previews, one of which is bookmarked.](/static/topic/libraries/architecture/images/mad-arch-ui-basic-case-study.png)
...
[Jetpack Compose](/compose) is the recommended toolkit.
Because the role of the [data
layer](/jetpack/guide/data-layer) is to hold, manage, and provide access to the
...
2. Consume UI-renderable data and transform it into UI elements for presentation
   to the user.
3. Consume user input events from those assembled UI elements and reflect their
   effects in the UI data as needed.
...
* Unidirectional data flow (UDF) as a means of producing and managing the UI
  state
...
In the [case study](#case-study) outlined earlier, the UI shows
...
![UI is a result of binding UI elements on the screen with the UI state.](/static/topic/libraries/architecture/images/mad-arch-ui-elements-state.png)


**Figure 3.** UI is a result of binding UI elements on the screen with the
UI state.
...
see [State and Jetpack Compose](/develop/ui/compose/state).
...
**Key Point:** Only sources or owners of data should be responsible for updating the
data they expose.
...
[bottom app bar](/develop/ui/compose/components/app-bars#bottom) to a whole screen or a navigation destination.

In the latter case, the typical implementation is an instance of a
[ViewModel](/topic/libraries/architecture/viewmodel), although depending on the
requirements of the application, a simple class might suffice. The News app from
the [case study](#case-study), for example, uses a `NewsViewModel` class as a
state holder to produce the UI state for the screen displayed in that section.

**Key Point:** The [`ViewModel`](/topic/libraries/architecture/viewmodel) type is
the recommended implementation for the management of screen-level UI state with
access to the data layer. Furthermore, it survives configuration changes
automatically. `ViewModel` classes define the logic to be applied to events in
...
![Application data flows from the data layer to the ViewModel. UI state
    ...
    elements back to the ViewModel.](/static/topic/libraries/architecture/images/mad-arch-ui-udf.png)
...
* The ViewModel holds and exposes the state to be consumed by the UI. The UI
  state is application data transformed by the ViewModel.
...
[case study](#case-study) mentioned earlier contains a list of articles, each
...
![A single article item from the case study app. The UI shows a thumbnail, the article title, the author, the estimated reading time for the article, and a bookmark icon.](/static/topic/libraries/architecture/images/mad-arch-ui-basic-case-study-item.png)
...
![A UI event occurs when the user bookmarks an artcile. The ViewModel
    ...
    new UI state and passes it to the UI elements for display.](/static/topic/libraries/architecture/images/mad-arch-ui-udf-in-action.png)
...
layer](/jetpack/guide/data-layer) page. However, there are different types of
...
* **UI behavior logic** or **UI logic** is *how* to display state changes on
  the screen. Examples include obtaining the right text to show on the screen
  using Android [`Resources`](/reference/android/content/res/Resources),
  navigating to a particular screen when the user clicks a button, or
  displaying a user message on the screen using a
  [toast](/guide/topics/ui/notifiers/toasts) or a
...
UI types like [`Context`](/reference/android/content/Context).
...
guide](/jetpack/compose/state#managing-state).
...
* **Maintainability.** Mutation of state follows a well-defined pattern where
  mutations are a result of both user events and the sources of data they pull
  from.
...
For an introduction to Kotlin flows, see [Kotlin flows on Android](/kotlin/flow).
To learn how to use `StateFlow` as an observable data holder,
see the
[Advanced State and Side Effects in Jetpack Compose](/codelabs/jetpack-compose-advanced-state-side-effects) codelab.

**Note:** In Jetpack Compose apps, you can use Compose's observable [State
APIs](/jetpack/compose/state#state-in-composables) such as `mutableStateOf` or
`snapshotFlow` for the exposure of UI state. Any type of observable data holder
such as `StateFlow` that you see in this guide can be consumed in Compose using
the appropriate [extensions](/jetpack/compose/libraries#streams).
...
You can launch a coroutine using the [`viewModelScope`](/topic/libraries/architecture/coroutines#viewmodelscope),
...
[Show errors on the screen](#show-errors) section.

**Note:** The pattern shown in the previous example, where state is mutated through
functions on the ViewModel, is one of the more popular implementations of
...
see [Lifecycle of composables](/develop/ui/compose/lifecycle).
...
elements like [snackbars](/develop/ui/compose/components/snackbar).
For more information on how UI events are produced and consumed, see [UI
events](/jetpack/guide/ui-layer/events).
...
see [Kotlin coroutines on Android](/kotlin/coroutines).
...
like the ones covered in the earlier [Consume UI state](#consume-ui-state) section,
but defer the consumption implementation to the [Navigation component](/guide/navigation).

For more information about UI navigation,
see [Navigation 3](/guide/navigation/navigation-3).

## Paging

The [Paging library](/topic/libraries/architecture/paging/v3-overview) is
...
see [Navigation 3](/guide/navigation/navigation-3/animate-destinations) and
[Shared element transitions in Compose](/develop/ui/compose/animation/shared-elements).

## Additional resources

### Views content

* [UI layer (Views)](/topic/architecture/views/ui-layer)
