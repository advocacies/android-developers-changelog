---
title: Create ViewModels with dependencies (Views)  |  Android Developers
...
# Create ViewModels with dependencies (Views)   Part of [Android Jetpack](/jetpack).

[Concepts and Jetpack Compose implementationarrow\_forward](/topic/libraries/architecture/viewmodel/viewmodel-factories)
...
it's the case for the [Architecture Blueprints](https://github.com/android/architecture-samples/blob/views/app/src/main/java/com/example/android/architecture/blueprints/todoapp/ViewModelFactory.kt) sample.
...
[`ViewModelStoreOwner`](/reference/androidx/lifecycle/ViewModelStoreOwner) such as
...
factories from a subset of classes that extend [`ViewModelProvider.Factory`](/reference/androidx/lifecycle/ViewModelProvider.Factory)
...
* [`AndroidViewModelFactory`](/reference/androidx/lifecycle/ViewModelProvider.AndroidViewModelFactory) if the `Application` class is needed.
* [`AbstractSavedStateViewModelFactory`](/reference/androidx/lifecycle/AbstractSavedStateViewModelFactory) if [`SavedStateHandle`](/reference/kotlin/androidx/lifecycle/SavedStateHandle) needs
  to be passed as a dependency.

If `Application` or `SavedStateHandle` aren't needed, simply extend from
`ViewModelProvider.Factory`.

The following example uses an [`AbstractSavedStateViewModelFactory`](/reference/androidx/lifecycle/AbstractSavedStateViewModelFactory) for a
ViewModel that takes a repository and a [`SavedStateHandle`](/reference/kotlin/androidx/lifecycle/SavedStateHandle) type as a
dependency:
...
**Warning:** if you're using a version of `ViewModel` older than 2.5.0, you might
have the option to override a create function with `CreationExtras` in its
signature. Don't override that function. Instead, override the `create` function
that has the `key, modelClass` and `savedStateHandle` parameters.
