---
title: Upgrade dependency versions  |  Android Studio  |  Android Developers
url: https://developer.android.com/build/version-upgrade-strategies
source: html-scrape
---
...
**Note:** Before updating your dependencies, consider enabling
[Dependency verification](/build/dependency-verification) to help verify the dependencies you download
...
understand how [Gradle resolves the versions you request](/build/gradle-dependency-resolution), the risks that
are involved, and steps you can take to mitigate those risks.

**Note:** Good [app tests](/training/testing) are key to ensuring upgrades don't break your build
or app in unexpected ways.
...
build elements in [Tool and library interdependencies](/build/tool-and-library-dependencies).

![Build dependencies and their relationships](/static/images/build/build-dependencies.svg)
...
|  |  |
| --- | --- |
| [Android Gradle Plugin (AGP)](/build/releases/gradle-plugin) | Android Studio includes an [AGP upgrade assistant](/build/agp-upgrade-assistant) that can assist with these tasks.  If you use the assistant or perform the upgrade manually, consider the following:  Look at the [AGP release notes](/build/releases/gradle-plugin).  [Upgrade Gradle](/build/releases/gradle-plugin#updating-gradle) to at least the version listed.  Upgrade [Android Studio](/build/releases/gradle-plugin#android_gradle_plugin_and_android_studio_compatibility) to a version that supports the chosen AGP version.  Use versions of Android Studio and AGP that [support the Android SDK](/build/releases/gradle-plugin#api-level-support) you want to use.  Check compatibility with SDK Build Tools, NDK, and JDK.  If you develop a Gradle plugin (for internal or public use) that extends or uses data from AGP, check whether you need to upgrade your plugin. Sometimes AGP deprecates and later removes APIs, causing incompatibilities with previous plugins. |
| [Kotlin compiler, language and runtime](https://kotlinlang.org/) | Check the [Kotlin release notes](https://kotlinlang.org/docs/releases.html#release-details) for known issues and incompatibilities.  If you use Jetpack Compose:   * If the new Kotlin version is lower than 2.0.0:  + Locate the [compatible Compose compiler plugin](/jetpack/androidx/releases/compose-kotlin#pre-release_kotlin_compatibility). + Upgrade `kotlinCompilerExtensionVersion` in the `build.gradle.kts` of any modules where it appears.  * If the new Kotin version is 2.0.0 or higher:  + Set up the [Compose Compiler Gradle Plugin](/develop/ui/compose/compiler) with the same version as the Kotlin compiler.   If you use Kotlin Symbol Processing (KSP), see [KSP Quickstart](https://kotlinlang.org/docs/ksp-quickstart.html) for setup and [KSP Releases](https://github.com/google/ksp/releases) for available versions. Note that you must use a version of KSP that matches the Kotlin version. For example, if you're using Kotlin 2.0.21, you can use any version of the KSP plugin that starts with 2.0.21, such as 2.0.21-1.0.25. You usually won't need to upgrade the KSP processors (such as the [Room compiler](/jetpack/androidx/releases/room), which appears as a `ksp` dependency in your build files); the KSP plugin abstracts much of the compiler API, and the KSP API used by processors is stable.  Upgrade all other Kotlin Compiler Plugins that you're using. The Kotlin Compiler Plugin API often changes between releases, and the plugins must use a compatible API. If the plugin is listed in [Compiler plugins](https://kotlinlang.org/docs/all-open-plugin.html), you must use the same version as the Kotlin compiler. For any other compile plugins, check their documentation for the appropriate mapping.  Note that compiler plugins that are not maintained alongside the Kotlin compiler itself often experience release delays as they wait for the compiler plugin API to stabilize. Before upgrading Kotlin, check that all compiler plugins you use have matching upgrades available.  Finally, on some occasions, the Kotlin language changes, requiring you to update your code. This most often happens if you're trying out experimental features. If your code doesn't build properly after upgrading the Kotlin compiler, check for language changes or runtime library breakage in the [Kotlin release notes](https://kotlinlang.org/docs/releases.html#release-details). |
...
| [Android SDK](/about) | Android Studio includes an [Android SDK upgrade assistant](/build/sdk-upgrade-assistant) that can help with these tasks.  If you use the assistant or perform the upgrade manually, consider the following:  Each release of the Android SDK contains new features and APIs, bug fixes and behavior changes. The Play Store [requires updating your `targetSdk`](/google/play/requirements/target-sdk), but consider updating `targetSdk` earlier than the deadlines to allow more time to make any necessary changes.  Before upgrading the Android SDK, carefully read the [release notes](/about/versions). Pay close attention to the behavior changes section, which includes:   * New permissions that you'll need to request at install or runtime. * Deprecated APIs and their replacements. * Breaking changes in APIs or behavior. * New Kotlin or Java APIs, which may impact your code.   The behavior changes section can be quite long, but pay close attention as it often contains critical changes you need to make to your application.  You must upgrade [`targetSdk`](/build#android_sdk_settings) to meet Play Store requirements. Upgrading [`compileSdk`](/build#android_sdk_settings) is optional, giving you access to new APIs. Note that some libraries, like AndroidX, include a minimum [`compileSdk`](/build#android_sdk_settings) requirement.  To take advantage of new SDK features during development and ensure compatibility during your build, upgrade the Android Gradle plugin (AGP) and Android Studio. These include new and improved tools for new SDKs. See [Minimum versions of tools for Android API level](/build/releases/gradle-plugin#api-level-support).  When upgrading the Android SDK, upgrade any AndroidX libraries that you use. AndroidX often uses new and updated APIs for better compatibility and performance across Android SDK versions. |
| [Android Studio](/studio) | You can generally upgrade Android Studio at any time. You might see messages prompting you to [upgrade AGP](/build/agp-upgrade-assistant) or [upgrade the Android SDK](/build/sdk-upgrade-assistant). These upgrades are strongly recommended but not required.  If you later want to use Android Studio to upgrade AGP or the Android SDK, you can find these options on the **Tools** menu:   * [**AGP Upgrade Assistant…**](/build/agp-upgrade-assistant) * [**Android SDK Upgrade Assistant…**](/build/sdk-upgrade-assistant) |
| [Java](https://www.java.com/) | If you have Java source code in your Android application, you may want to take advantage of newer Java APIs.  Each Android SDK version supports a [subset of Java APIs](/studio/write/java8-support-table) and language features. AGP provides compatibility for lower Android SDK versions using a process called [desugaring](/studio/write/java8-support).  Android SDK [release notes](/about/versions) specify which Java level is supported and potential issues. Some of these issues may impact Kotlin source code as well, because Kotlin has access to the same Java APIs. Be sure to pay close attention to JDK API sections that appear in the behavioral changes section of the release notes, even if you have no Java source code.  JDK usage is specified in several places in your build scripts. See [Java versions in Android build](/build/jdks) for more information. |
...
**Caution:** Dependency Guard is a third-party Gradle plugin. It's possible for
malicious actors to compromise external plugins using man-in-the-middle attacks
or through a hacked development server. We strongly recommend enabling
[Dependency verification](/build/dependency-verification) to help verify the dependencies you download
and include in your project.
...
* Lint
  ...
  + Or, if you have many existing warnings, create an
    [Android lint baseline](/studio/write/lint#snapshot). Over time, be sure to clean up existing Warnings
    (or suppress those you don't care about)
...
| READMEs | Some README files for a library note potential risks, especially if the library doesn't provide release notes. Look for \_known issues\_, especially known security concerns. |
...
| Maintenance and quality risks | For libraries with public repositories:   * How many contributors are maintaining the library? * When was the last upgrade, and how often does the library change? * What does the issue backlog (if available) look like? Skim it to get a feeling for potential problems and the technical debt of the library. * How well do unit tests cover the library? * Are there known anti-patterns in the codebase? * Is the library well documented? * Are there many \_fixme\_ comments in the codebase? |
...
[Android lint](/studio/write/lint) can catch many issues in your application, including some
that are the result of changing versions of dependencies or the Android SDK. For
example, if you upgrade your [`compileSdk`](/build#android_sdk_settings) and use its new APIs, lint
...
If you use [Continuous Integration](/training/testing/continuous-integration) (CI), run `gradlew
...
**Note:** General and Android SDK–level lint rules are delivered with AGP, and
library-specific rules are packaged with their libraries. Keep AGP and your
library dependencies as up to date as possible to ensure that the latest lint
rules are applied to your code. This is especially important for new lint rules
that have been added or upgraded for a new Android SDK version.
...
* Accept some risks as is. Some risks are low enough to be acceptable,
  especially when upgrade time and resources are limited.
* Reject some risks outright. Some upgrades might feel too risky, especially if
  you have limited time or resources to mitigate them at this point. If you need
  to triage, focus on upgrades that are necessary for bugs you have encountered
  or new features you need.
* Mitigate remaining risks
  + Consider batching your upgrades into smaller, independent sets of changes.
    This reduces overall risk and allows partial rollback.
  + Investigate the changes in detail.
  + [Test your app](/training/testing) to check for unexpected changes. Add new tests where
    needed to build confidence in the upgrade.
...
* Reach out to the library owner and request continuation of the existing
  license or dual licensing to keep allowing the old license.
* Work with your legal team to determine if you can change your license to be
  compatible.
* Find another library with a compatible license and modify your application as
  needed.
* Fork the last compatible version of the library (if that license allows
  derivatives and the changes are not retroactive) and make your own changes.