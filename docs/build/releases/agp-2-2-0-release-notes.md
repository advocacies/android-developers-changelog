...
| --- | --- | --- | --- |
| Gradle | 2.14.1 | 2.14.1 | To learn more, see [updating Gradle](/build/releases/gradle-plugin?buildsystem=ndk-build#updating-gradle). |
...
:   * Uses Gradle 2.14.1, which includes performance improvements
    ...
    * Using the [`externalNativeBuild {}`](https://google.github.io/android-gradle-dsl/current/com.android.build.gradle.internal.dsl.ExternalNativeBuild.html) DSL, Gradle now lets you
      link to your C/C++ sources and compile C/C++ libraries using
      CMake or ndk-build. After building your C/C++ libraries, Gradle
    ...
    * When you [run a
      build from the command line](/studio/build/building-cmdline), Gradle now attempts to auto-download
    ...
    * Improves build performance by adopting a new default packaging
      pipeline which handles zipping, signing, and [zipaligning](/studio/command-line/zipalign) in one task. You
      can revert to using the older packaging tools by adding
      `android.useOldPackaging=true` to your
      `gradle.properties` file. While using the new packaging
      tool, the `zipalignDebug` task is not available. However,
    ...
    * APK signing now uses [APK Signature Scheme
      v2](/about/versions/nougat/android-7.0#apk_signature_v2) in addition to standard JAR signing. All Android platforms accept the
    ...
    * For multidex builds, you can now use ProGuard rules to determine which
      ...
      `buildTypes.multiDexKeepProguard`. Using this DSL is different
      from using
      [`buildTypes.proguardFiles`](https://google.github.io/android-gradle-dsl/current/com.android.build.gradle.internal.dsl.BuildType.html#com.android.build.gradle.internal.dsl.BuildType:proguardFiles(java.lang.Object[])), which provides general ProGuard
      rules for your app and does not specify classes for the main DEX file.
    * Adds support for the `android:extractNativeLibs` flag,
      which can reduce the size of your app when you install it on a
      device. When you set this flag to `false` in the
      [`<application>`](/guide/topics/manifest/application-element)
      element of your app manifest, Gradle packages uncompressed and
      aligned versions of your C/C++ libraries with your APK. This
      prevents [`PackageManager`](/reference/android/content/pm/PackageManager)
      from copying out your C/C++ libraries from the APK to the device's
...
Changes:
:   * `getDefaultProguardFile` now returns the default ProGuard
    ...
    * Improved Jack compiler performance and features:
      ...
      + Improved support for annotation processors. Annotation
        processors on your classpath, such as any `compile`
        dependencies, are automatically applied to your build. You can
        also specify an annotation processor in your build and pass
        arguments by using the [`javaCompileOptions.annotationProcessorOptions {}`](https://google.github.io/android-gradle-dsl/current/com.android.build.gradle.internal.dsl.AnnotationProcessorOptions.html) DSL in your
