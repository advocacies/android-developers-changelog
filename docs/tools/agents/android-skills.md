---
title: Overview of Android skills  |  Android Studio  |  Android Developers
...
# Overview of Android skills Stay organized with collections Save and categorize content based on your preferences.
...
[Android CLI](/tools/agents/android-cli) to install skills for use with any agent and
tooling of your choice.

**Note:** How to structure and use skills might differ slightly based on the agent
and development environment you're using. Be sure to check the documentation
for your agent and tooling.
...
* **Ground with expert knowledge:** Provide more Android-specific context to
  agents, grounding them on demand and extending their default knowledge and
  expertise beyond the regular model training cycle.
* **Repeatable workflows:** Provide standard instructions to help ensure that
  multi-step tasks in Android development are performed consistently.
* **Resource bundling:** Include scripts, templates, or additional
  documentation alongside your main SKILL.md instructions so the agent has
  everything it needs in one place, avoiding the need to manually attach files
  for a prompt.
* **Shared expertise:** Package your team's Android development patterns into
  shared folders for unified access and collaboration.
...
![gemini_cli_skills_demo.gif](/static/tools/agents/videos/gemini_cli_skills_demo.gif)

### Android CLI

We recommend installing an Android skill using the
[Android CLI](/tools/agents/android-cli), which makes it easier to discover, download,
and manage skills for any agent of your choice.

* To see the list of available skills, run [`android skills list`](/tools/agents/android-cli#skills-list).
* To install a skill, run [`android skills add --skill skill-name`](/tools/agents/android-cli#skills-add).
  If you already have the skill installed, `skills add` will update it to the latest version.
...
[import it into your project in Android Studio](/studio/gemini/skills).
...
In [Android Studio](/studio/gemini/skills), you can also invoke a skill
...
* **Scope:** Only skills located within the project's codebase are
  supported.

![Directory structure showing the location of skill files in a project](/static/studio/images/skill-files.png)
