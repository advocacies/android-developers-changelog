---
title: GameTextInput  |  Android game development  |  Android Developers
...
# GameTextInput   Part of [Android Game Development Kit](/games/agdk/overview).

Using the [`GameTextInput`](/reference/games/game-text-input) library is a
...
(IME)](/guide/topics/text/creating-input-method) features such as spell-
checking, completions, and multi-key characters.

Internally, `GameTextInput` accumulates the input text (together with the
relevant states) to the internal buffer [`GameTextInput::currentState_`](/games/agdk/reference/games/game-text-input/struct/game-text-input-state) and notifies
...
* Together with GameActivity: GameActivity integrates GameTextInput.
  ...
  [the GameActivity page](/games/agdk/game-activity/use-text-input) . For a
...
`GameTextInput` is distributed as an [Android Archive (AAR)](/studio/projects/android-library). This AAR contains the Java classes and
...
[`Prefab`](/studio/build/dependencies#using-native-dependencies),
which exposes native libraries and source code to your [CMake project](/ndk/guides/cmake) or [NDK build](/ndk/guides).

1. Follow the instructions on the
   [Jetpack Android Games](/jetpack/androidx/releases/games) page to add the
...
1. From your C thread that is already attached to the JVM, or the app main
   thread, call [`GameTextInput_init`](/games/agdk/reference/games/game-text-input/group/game-text-input#gametextinput_init)
   with a [`JNIEnv`](/training/articles/perf-jni#javavm-and-jnienv) pointer.
...
4. Retrieve this new `InputEnabledTextView` class to your Java activity. This is
   relative simple when you use [View Binding](/topic/libraries/view-binding):
...
5. In your C library, pass `inputConnection` into
   [`GameTextInput_setInputConnection`](/reference/games/game-text-input/group/game-text-input#gametextinput_setinputconnection).
   Pass a callback in
   [`GameTextInput_setEventCallback`](/reference/games/game-text-input/group/game-text-input#gametext-input_seteventcallback)
   to be notified of events as C state struct [`GameTextInputState`](/reference/games/game-text-input/struct/game-text-input-state).
...
6. In your C library, call
   [`GameTextInput_processEvent`](/reference/games/game-text-input/group/game-text-input#gametextinput_processevent)
...
and hiding the IME through the [`GameTextInput_showIme`](/reference/games/game-text-input/group/game-text-input#gametextinput_showime)
and [`GameTextInput_hideIme`](/reference/games/game-text-input/group/game-text-input#gametextinput_hideime)
functions.
...
* [Use GameTextInput with GameActivity](/games/agdk/game-activity/use-text-input)
* [GameTextInput Reference doc](/reference/games/game-text-input)
