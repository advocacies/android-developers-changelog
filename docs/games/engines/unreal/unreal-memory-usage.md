---
title: Check memory usage with Unreal Engine  |  Android game development  |  Android Developers
...
# Check memory usage with Unreal Engine Stay organized with collections Save and categorize content based on your preferences.





Representing the memory pressure of a process, '[**anonymous RSS + swap**](/topic/performance/vitals/memory-usage)'
...
* [Query memory using C++ APIs](#query-memory-cpp)
* [Monitor memory using console commands](#monitor-console-commands)
* [Visualize memory with Unreal Insights](#visualize-unreal-insights)
* [Verify precise memory usage using Perfetto](#verify-precise-memory-usage)
...
![Unreal Engine stat unit overlay showing the Mem row.](/static/images/games/engines/unreal/unreal-memory-usage-1.png)


**Figure 1.** The `stat unit` overlay
displaying the `Mem` row.
...
1. Connect your device to the Memory Trace Channels with flags that
   include `default,counters` to view the `PlatformMemory/UsedPhysical`
   (`VmRSS + VmSwap`) graph alongside the frame timeline.
2. Optional: To track custom memory metrics in more detail, declare and
   update a custom trace counter using `TRACE_DECLARE_MEMORY_COUNTER` and
   `TRACE_COUNTER_SET`.
...
![Unreal Insights showing Memory/EstimatedPhysicalMemory.](/static/images/games/engines/unreal/unreal-memory-usage-2.png)


**Figure 2.** The Unreal Insights timeline showing the
custom `Memory/EstimatedPhysicalMemory` counter.

## Verify precise memory usage using Perfetto

To verify exact Android OS '**anonymous RSS + swap**' values rather than an
estimate, use [Perfetto](/tools/perfetto). Perfetto captures system-level memory metrics
to evaluate your game's exact footprint.

![Perfetto UI showing anonymous RSS, swap, and their sum query.](/static/images/games/engines/unreal/unreal-memory-usage-3.png)


**Figure 3.** The Perfetto UI showing anonymous RSS, swap, and their sum query.
